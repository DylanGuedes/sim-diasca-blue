...
%
% - data_filename ::file_utils:file_name() is the name of the time-series data
% file (.dat), as a plain string
%
...
curve_to_string( #curve_metadata{
	...
	MinString = case Min of
		...
		{ MinValue, MinTick } ->
			io_lib:format( " It reached its minimum, ~f, at tick #~B.",
						   [ MinValue, MinTick ] )
	...
	MaxString = case Max of
		...
		{ MaxValue, MaxTick } ->
			io_lib:format( " It reached its maximum, ~f, at tick #~B.",
						   [ MaxValue, MaxTick ] )
	...
	FilterString = case Filters of
		...
		_ ->
			io_lib:format( " Associated to following curve filters: ~w",
						   [ Filters ] )
