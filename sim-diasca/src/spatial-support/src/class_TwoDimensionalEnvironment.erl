...
% be managed
%
% - entities :: table:table( pid(), entity() ) is a table associating the entity
% knowledge of this environment (as a record) to the PID of this entity
%
% - query_table :: table:table( pid(), query() ) is the
...
% stored query
%
% - request_table :: table:table( pid(), [ pid() ] ) is a
...
-spec declareEntity( wooper:state(), position(),
				 unit_utils:meters_per_second() | 'undefined', class_name(),
				 actor_pid() ) -> class_Actor:actor_oneway_return().
declareEntity( State, CurrentPosition, MaxSpeed, Classname,
	...
	NewEntities = table:addEntry( _K=SpatialisedActorPid,
...
undeclareEntity( State, SpatialisedActorPid ) ->
	...
	NewEntityTable = table:removeEntry( _K=SpatialisedActorPid,
										?getAttr(entities) ),
...
getEntitiesWithin( State, Position, Radius, SpatialisedActorPid ) ->
	...
	FullEntityTable = table:addEntry( SpatialisedActorPid,
	...
	NewState = case lists:foldl(
		...
		{ EntitiesIn, EntitiesMaybeIn } ->
					...
					case table:hasEntry( SpatialisedActorPid,
										 QueryTable ) of
					...
					NewQTable = table:addEntry( SpatialisedActorPid,
												Query, QueryTable ),
...
getTypedEntitiesWithin( State, TargetClass, Position, Radius,
	...
	NewState = case lists:foldl(
		...
		{ EntitiesIn, EntitiesMaybeIn } ->
			...
			case table:hasEntry( SpatialisedActorPid, QueryTable ) of
			...
			NewQTable = table:addEntry( SpatialisedActorPid, Query,
										QueryTable ),
...
notifyPosition( State, Position, EntityPid ) ->
	...
	{ RequesterList, ShrunkRequestTable} = table:extractEntry(
	...
	{ FoldState, FoldQueryTable } = lists:foldl(
		   ...
		   fun( RequesterPid, { AccState, AccQueryTable } ) ->
				...
				{ Query, NewQueryTable } = table:extractEntry(
	...
	NewEntityTable = table:addEntry( EntityPid, NewEntityRecord,
									 ShrunkEntityTable ),
...
update_position( EntityPid, Position, CurrentTickOffset, EntityTable ) ->
	...
	{ EntRecord, ShrunkEntityTable } = table:extractEntry( EntityPid,
														   EntityTable ),
...
declare_requester( RequestingEntityPid, EntityPid, RequestTable ) ->
	...
	%
	case table:lookupEntry( _K=EntityPid, RequestTable ) of

		key_not_found ->
			% So add a new entry:
			table:addEntry( EntityPid, [ RequestingEntityPid ],
							RequestTable );
...
update_query( Query=#disc_query{ center=Center, radius=Radius, entities_in=Ins,
	...
	case list_utils:delete_existing( EntityPid, Requested ) of
		...
		RequesterList ->
			...
			UpdatedQueryTable = table:addEntry( RequesterPid,
