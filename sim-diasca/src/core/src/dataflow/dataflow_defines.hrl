...
%
-type string_iteration_name() :: string().
...
%
-type port_spec_type() :: 'input_port_iteration' | 'output_port_iteration'
						| port_type().
...
% User-level counterpart:
-type iteration_port_string_target() :: { block_pid(),
										  input_iteration_string_name() }.
...
% 'Common' layer, and to unit_utils.erl for more details.
%
...
% boolean.
%
-type value_type_description() :: meta_utils:type_description().
...
-record( input_port_spec, {
		   ...
		   comment = undefined :: 'undefined' | port_comment(),
...
-record( dataflow_attribute_spec, {
		...
		%
		comment :: 'undefined' | dataflow_attribute_comment(),
...
%
-type block_type() :: wooper:class_name().
...
%
-type object_manager_name() :: wooper:class_name().


% Name of a dataflow unit manager, i.e. its classname (ex:
% 'class_UrbanUnitManager').
%
-type unit_manager_name() :: wooper:class_name().
...
-type identification_manager_pid() :: agent_pid().
