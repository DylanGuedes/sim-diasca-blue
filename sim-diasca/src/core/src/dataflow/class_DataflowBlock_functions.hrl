...
% For most validate_X (used for standard ports), a get_X counterpart (used for
% iterated ports) is defined, in order to be able to assign with no extra
% validation values that have already been statically checked with iterations.
...
validate_semantics( Semantics, SemanticServerPid, State )
  when is_list( Semantics ) ->
	...
	% strings:
	%
	BinSemantics = case text_utils:is_list_of_strings( Semantics ) of
		...
		true ->
			...
			ActualUnit = try
				...
				E ->
					...
					PlaceholderUnit = unit_utils:parse_unit(
										"invalid unit specified" ),

					?warning_fmt( "Parsing of port unit '~s' failed: '~p', "
								  "injecting instead unit '~s'.",
								  [ Unit, E, unit_utils:unit_to_string(
											   PlaceholderUnit ) ] ),
			...
			%trace_utils:debug_fmt( "Parsed unit '~s' as '~s'.",
...
validate_type_description( _TypeDescription, _TypeServerPid, _State ) ->
	...
	% Not implemented yet; will parse as much as possible the description, try
	% to resolve it locally, and, only if necessary (typically if depending on
	% types that are not known locally), will request the type server to resolve
	% it instead.
...
connect_to_input_port( InputPortBinName, InputPortTable,
	...
	InputPort = case table:lookupEntry( InputPortBinName, InputPortTable ) of

		{ value, IPort } ->
			IPort;

		key_not_found ->
			?error_fmt( "Request to connect a non-existing input port '~s', "
						"whereas ~s", [ InputPortBinName,
	...
	case InputPort#input_port.feeder_port of
		...
		FeederPortId={ CurrentUpstreamBlockPid, CurrentOutputPortBinName } ->
			...
			InitiatorString = case InitiatorInfo of
				...
				CallerPid ->
					text_utils:format( "~w to connect the output port '~s' "
							"of block ~w",
							[ CallerPid, OutputPortBinName, UpstreamBlockPid ] )

			end,

			?error_fmt( "Request from ~s to local input port '~s', whereas "
			...
			% Input port specified, then { currently linked output one,
			% requested new one }:
	...
	case can_be_connected( OutputPortDescription, OutputPortBinName,
		...
		true ->
			?trace_fmt( "Connection of output port '~s' (to input port ~s) "
						"granted.",
						[ OutputPortBinName, InputPortBinName ] );

		{ true, WarningMsg } ->
			?warning_fmt( "Connection from output port ~w:'~s' to "
						  "local input port '~s' accepted, yet ~s.",
						  [ UpstreamBlockPid, OutputPortBinName,
							InputPortBinName, WarningMsg ] );

		{ false, Reason } ->
			?error_fmt( "Error, connection attempt from output port ~w:'~s' to "
						"local input port '~s' rejected (~s), reason: ~p.",
	...
	?trace_fmt( "Input port '~s' now fed by output port '~s' of "
				"upstream block ~w.",
				[ InputPortBinName, OutputPortBinName, UpstreamBlockPid ] ),

	% Returns a new input port table:
	table:updateEntry( InputPortBinName, NewInputPort, InputPortTable ).
...
% extent (some may be statically incompatible; ex: 'lower than X' versus 'higher
% than X+1').
%
...
can_be_connected( OutputPortDescription=#port_description{
	...
	%
	% The constraints as such are ignored (yet will be checked on a per-value
	% basis).
	%
	case check_semantics( OutputPortSemantics, InputPortSemantics,
		...
		SemAccepted ->
			...
			case unit_utils:are_units_identical( OutputPortCanonicalUnit,
				...
				true ->
					case meta_utils:are_types_identical( OutputPortType,
														 InputPortType ) of
				...
				false ->
					 { false, { unmatching_units, OutputPortBinUnit,
								InputPortBinUnit } }
...
check_semantics( OutputPortSemantics, InputPortSemantics, OutputPortDescription,
	...
	case class_SemanticServer:are_semantics_compliant(
		...
		false ->
			...
			LackString = text_utils:binaries_to_string(
						   lists:sort( NotCoveredSemList ) ),
			...
			WarningMsg = text_utils:format(
					"the semantics for upstream output port '~s' are not "
					"compliant with the ones of the local input port '~s', "
					"as following ~B semantics are lacking:~s~n."
					"More precisely, regarding output a ~s would have been "
					"connected to an ~s, knowing the output port has a ~s, "
					"and the input port has a ~s.",
					[ OutputPortBinName, InputPortBinName,
					  length( LackString ), LackString,
					  port_description_to_string( OutputPortDescription ),
...
% Ensures that any class-level information about semantics and types match the
		...
		UserVocabulary ->
			...
			case check_output_semantics( Vocabulary, OutputTable ) of
				...
				{ OPortName, OPortSemantics } ->
					?error_fmt( "The '~s' output port declared the '~s' "
...
input_port_spec_to_string( #input_port_spec{
	...
	% Not an iteration:
	text_utils:format( "specification for a standard input port named '~s' "
					   "(comment: '~s'), of semantics '~p', "
					   "user-defined unit '~s', type description '~s' and "
					   "constraints '~p'.",
...
input_port_spec_to_string( #input_port_spec{
	...
	text_utils:format( "specification for an input port iteration named '~s' ~s"
					   " (comment: '~s'), of semantics '~p', user-defined unit "
					   "'~s', type description '~s' and constraints '~p'.",
...
output_port_spec_to_string( #output_port_spec{
	...
	% Not an iteration:
	text_utils:format( "specification for a standard output port named '~s' "
					   "(comment: '~s'), ~s, of semantics '~p', user-defined "
					   "unit '~s', type description '~s' and constraints '~p'.",
...
output_port_spec_to_string( #output_port_spec{
	...
	text_utils:format( "specification for an output port iteration named '~s' "
					   "~s (comment: '~s'), ~s, of semantics '~p', "
					   "user-defined unit '~s', type description '~s' and "
					   "constraints '~p'.",
...
to_string( State ) ->
	...
	text_utils:format( "dataflow block named '~s', ~s, having ~sand ~s~s",
...
io_to_string( IndentationLevel, State ) ->
	...
	InputDetailed = case SortedInputs of
		...
		[ { SingleIName, SingleIDesc } ] ->
			text_utils:format( "a single ~s ", [ input_port_to_string(
	...
	OutputDetailed = case SortedOutputs of
		...
		[ { SingleOName, SingleODesc } ] ->
			text_utils:format( "a single ~s ", [ output_port_to_string(
...
input_port_to_string( PortName, _InputPort=#input_port{
	...
	FeederString = case Feeder of
		...
		{ OutputBlockPid, OutputPortName }  ->
			text_utils:format( "fed by output port ~s of dataflow block ~p",
							   [ OutputPortName, OutputBlockPid ] )

	end,

	text_utils:format( "input port named '~s', ~s, ~s, expecting values "
					   "~s, ~s, ~s; port is ~s, ~s, ~s",
...
output_port_to_string( PortName, _OutputPort=#output_port{
	...
	FedString = case FedPorts of
		...
		[ { InputBlockPid, InputPortName } ] ->
			text_utils:format( "feeding input port '~s' of dataflow block ~p",
							   [ InputPortName, InputBlockPid ] );

		_ ->
			FedStrings = [ text_utils:format( "port '~s' of dataflow block ~p",
					 [ PName, BName ] ) || { BName, PName } <- FedPorts ],

			text_utils:format( "feeding following ~B input ports:~s",
							   [ length( FedPorts ),
								 text_utils:strings_to_string( FedStrings ) ] )

	end,

	text_utils:format( "output port named '~s', ~s, ~s, expecting values "
					   "~s, ~s, ~s, ~s; port is ~s, ~s, ~s",
...
input_port_iteration_to_string( #input_port_iteration{
	...
	text_utils:format( "input port iteration having for base name '~s', "
					   "~s, ~s, ~s, ~s, ~s, ~s and following indexes of "
					   "iterated ports: ~w",
...
output_port_iteration_to_string( #output_port_iteration{
	...
	text_utils:format( "output port iteration having for base name '~s', "
					   "~s, ~s, ~s, ~s, ~s, ~s and following indexes of "
					   "iterated ports: ~w",
					   [ BaseName, CommentString, MultiplicityString,
						 SemanticString, UnitString, TypeString,
...
port_description_to_string( #port_description{
	...
	text_utils:format( "port with, as semantics, ~s, "
					   "and user-defined unit '~s' (corresponding to ~s), "
					   "type '~s' and constraints '~p'",
					   [ SemanticString, BinUnitString,
						 unit_utils:unit_to_string( CanonicalUnit ),
						 meta_utils:type_to_description( Type ),
						 Constraints ] ).
...
value_to_string( #channel_value{ actual_value=Value,
	...
	text_utils:format( "channel value '~p', having, as semantics, ~s, "
					   "and for unit '~s' (~s) and type '~s'",
...
value_to_string( #channel_value{ actual_value=Value,
	...
	text_utils:format( "invalid channel value '~p', having, as semantics, ~s, "
					   "and for unit '~p' and type '~p'",
					   [ Value, SemanticString, Unit, Type ] ).



% Section for turning data declared by block modellers into appropriate records.
...
parse_raw_input_port_spec( RawInputPortSpec ) ->
	...
	% Loads the specfied pairs in a table, for an easier look-up (actually a
	% no-op):
	%
	IPSTable = list_hashtable:new( RawInputPortSpec ),

	% Tries to assign all the input_port_spec fields from the specified pairs:
	%
	#input_port_spec{
	   name = list_hashtable:getEntry( input_port_name, IPSTable ),
	   comment = list_hashtable:getEntry( comment, IPSTable ),
	   is_iteration = list_hashtable:getEntry( is_iteration, IPSTable ),
	   value_semantics = list_hashtable:getEntry( value_semantics, IPSTable ),
	   value_unit = list_hashtable:getEntry( value_unit, IPSTable ),
	   value_type_description = list_hashtable:getEntry( value_type_description,
														 IPSTable ),
	   value_constraints = list_hashtable:getEntry( value_constraints,
													IPSTable ) }.
...
parse_raw_output_port_spec( RawOutputPortSpec ) ->
	...
	% no-op):
	%
	OPSTable = list_hashtable:new( RawOutputPortSpec ),

	% Tries to assign all the output_port_spec fields from the specified pairs:
	%
	#output_port_spec{
	   name = list_hashtable:getEntry( output_port_name, OPSTable ),
	   comment = list_hashtable:getEntry( comment, OPSTable ),
	   is_iteration = list_hashtable:getEntry( is_iteration, OPSTable ),
	   produces_result = list_hashtable:getValueWithDefaults( produces_result,
															  false, OPSTable ),
	   value_semantics = list_hashtable:getEntry( value_semantics, OPSTable ),
	   value_unit = list_hashtable:getEntry( value_unit, OPSTable ),
	   value_type_description = list_hashtable:getEntry( value_type_description,
														 OPSTable ),
	   value_constraints = list_hashtable:getEntry( value_constraints,
													OPSTable ) }.
