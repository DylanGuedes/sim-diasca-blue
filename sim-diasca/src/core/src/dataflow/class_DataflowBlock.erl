...
% Base class for all dataflow blocks, defining the constructs that are common to
% most of the actual parts constituting a dataflow.
%
% This is a specialization of the generic actor, meant to gather all traits that
% are shared by the various dataflow blocks.
%
% This class should provide most of the basics needed to properly describe
% notably all dataflow objects and units.
%
% When a dataflow is updated (typically based on changesets), the various blocks
% impacted are placed in a suspended state. They are made active again when the
% structure of the dataflow is stable once again, i.e. when all updates have
% been applied.
%
...
-define( wooper_method_export,
		 ...
		 createInputIteratedPorts/3, createOutputIteratedPorts/3,
		 createOutputPorts/2,

		 connectOutputPortInitially/4, requestConnectionToInputPortInitially/5,

		 connectOutputPort/5, requestConnectionToInputPort/7,

		 connectToDownstreamBlock/5, requestInboundConnections/5,
		 onInboundConnectionsCreated/5,

		 setInputPortValue/4, notifyNewInput/4, requestOutputPortStatus/3,
		 resume/2,

		 disconnectFromBlock/2
...
%
-export([ get_block_pid/2, get_upstream_blocks/1, get_downstream_blocks/1,
...
% while many verifications can be done statically, and once for all for
...
% interfaces (ex: thanks to static methods).
...
% Add get_declared_semantics/0, get_declared_types/0 in child classes if
% appropriate:
%
-define( wooper_static_method_export, declare_static_information_for/1 ).
...
% Describes the specification of an inbound connection request, in the context
% of an upstream block and a downstream one:
%
-type inbound_connection_spec() :: { output_port_name(), port_description(),
...
-type action_id() ::
		class_DataflowUnitManager:action_id().
...
-export([ create_input_iterated_ports/4, create_output_iterated_ports/3,
		  ...
		  get_all_input_iteration_values/2,
...
% Dataflow overall helpers:
-export([ get_static_information/1,
		  get_referenced_semantics/2, get_referenced_types/2 ]).
...
% Implementation notes:
%
% As it should be, base semantics and type management is done statically (hence
% once per type of dataflow block, thanks to block and instance managers for
% example).
%
% Dynamic, per-instance semantics and type management is done only when
% necessary, i.e. if (and when) port creations that are themselves dynamic and
% per-instance are done.
...
% be specified (ex: 'km', not { 'W', 'm'} ), whereas values can be compounded
...
%
% - input_ports :: input_port_table(), an associative table which stores the
% input ports of that dataflow block
%
% - output_ports :: output_port_table(), an associative table which stores the
% output ports of that dataflow block
%
% - input_iterations :: input_iteration_table() is a table whose keys are the
% names of the known input port iterations, their associated values being the
% records that correspond to these iterations
%
% - output_iterations :: output_iteration_table() is a table whose keys are the
% names of the known output port iterations, their associated values being the
% records that correspond to these iterations
%
% - run_status :: run_status() describes the current mode of operation of this
% block
%
% - dataflow_pid :: dataflow_pid() is the PID of the dataflow including this
% block
%
% - semantic_server_pid :: semantic_server_pid() is the PID of the semantic
% server, possibly requested by this block
%
% - type_server_pid :: type_server_pid() is the PID of the type server, possibly
% requested by this block
%
% - identification_server_pid :: identification_server_pid() is the PID of the
% identification server, possibly requested by this block to resolve identifiers
...
destruct( State ) ->
	...
	?trace_fmt( "Being deleted, while being in following state: ~s.",
				[ to_string( State ) ] ),
	...
	case InputPortSpecs of
		...
		L ->
			?void_fmt( "Dataflow block registering ~B overall input port "
						"specification(s):~s", [ length( L ),
								text_utils:strings_to_string( PortNames ) ] )

	end,

	register_input_ports( InputPortSpecs, ?getAttr(input_ports),
						  ?getAttr(input_iterations), SemanticServerPid,
						  TypeServerPid, State ).
...
register_input_ports( [ _InputPortSpec=#input_port_spec{
	...
	%?debug_fmt( "Registering a new direct, standard input port: ~s",
	...
	NewInputTable = table:addNewEntry( _K=PortName, _V=NewInputPort,
									   InputTable ),
...
register_input_ports( [ InputPortSpec=#input_port_spec{
	...
	?void_fmt( "Registering a new input port iteration: ~s",
			   [ input_port_spec_to_string( InputPortSpec ) ] ),
	...
	{ InitialCount, _Bounds } = CanonicalMultiplicity = validate_iteration(
												  IterationSpec, State ),
	...
	{ NewInputTable, NewIteration } = case InitialCount of
		...
		0 ->
			?void_fmt( "Input port iteration '~s' created, with no initial "
						"port.", [ Name ] ),
		...
		_ ->
			...
			?void_fmt( "Input port iteration '~s' created, with ~B initial "
	...
	NewInputIterationTable = table:addNewEntry( BaseIterPortName, NewIteration,
												InputIterationTable ),
...
createInputIteratedPorts( State, InputIterationName, PortCount ) ->
	...
	NewInputIterationTable = table:updateEntry( InputIterationName,
	...
	?wooper_return_state_result( NewState, NewIteratedPortNames ).
...
createOutputIteratedPorts( State, OutputIterationName, PortCount ) ->
	...
	NewOutputIterationTable = table:updateEntry( OutputIterationName,
...
register_output_ports( OutputPortSpecs, SemanticServerPid, TypeServerPid,
	...
	register_output_ports( OutputPortSpecs, ?getAttr(output_ports),
						   ?getAttr(output_iterations), SemanticServerPid,
						   TypeServerPid, State ).
...
register_output_ports( [ _OutputPortSpec=#output_port_spec{
	...
	%?debug_fmt( "Registering a new direct, standard output port: ~s",
	...
	NewOutputTable = table:addNewEntry( _K=PortName, _V=OutputPort,
										OutputTable ),
...
register_output_ports( [ OutputPortSpec=#output_port_spec{
	...
	?void_fmt( "Registering a new output port iteration: ~s",
				[ output_port_spec_to_string( OutputPortSpec ) ] ),
	...
	{ NewOutputTable, NewIteration } = case InitialCount of
		...
		0 ->
			?void_fmt( "Output port iteration '~s' created, with no initial "
						"port.", [ Name ] ),
		...
		_ ->
			...
			{ NewIterPortNames, NewInpTable, NewIter } =
				create_output_iterated_ports( InitialCount, BlankIteration,
											  OutputTable ),
			?void_fmt( "Output port iteration '~s' created, with ~B initial "
						"ports: ~p", [ Name, InitialCount, NewIterPortNames ] ),
	...
	NewOutputIterationTable = table:addNewEntry( BasePortName, NewIteration,
...
register_output_ports( _OutputPortSpecs=[ FaultyOutputPortSpecs | _T ],
					   _OutputTable, _OutputIterationTable, _SemanticServerPid,
					   _TypeServerPid, State ) ->
...
validate_port_specs( PortType, PortName, Comment, Semantics, Unit,
	...
	ValidatedComment = try
		...
		throw:CommentError ->
			?error_fmt( "Error while validating the comment '~s' of ~s: ~p.",
						[ Comment, get_port_description( PortType, PortName ),
						  CommentError ] ),
			throw( CommentError )
	...
	ValidatedConstraints = try
		...
		throw:ConstraintError ->
			ConstraintString = dataflow_support:value_constraint_to_string(
								 Constraints ),
			?error_fmt( "Error while validating the constraint '~s' of ~s: ~p.",
						[ ConstraintString,
						  get_port_description( PortType, PortName ),
						  ConstraintError ] ),
			throw( ConstraintError )
	...
	ValidatedUnit = try
		...
		throw:UnitError ->
			?error_fmt( "Error while validating the unit '~s' of ~s: ~p.",
						[ Unit, get_port_description( PortType, PortName ),
						  UnitError ] ),
			throw( UnitError )
	...
	ValidatedSemantics = try
		...
		throw:SemanticError ->
			...
			%
			?error_fmt( "Error while validating the semantics '~p' of ~s: ~p.",
						[ Semantics, get_port_description( PortType, PortName ),
						  SemanticError ] ),
			Semantics
			%throw( SemanticError )
	...
	ValidatedType = try
		...
		throw:TypeError ->
			?error_fmt( "Error while validating the type description '~s' of "
						"~s: ~p.", [ TypeDescription,
									 get_port_description( PortType, PortName ),
									 TypeError ] ),
			throw( TypeError )
...
% (helper)
%
-spec get_port_description( port_spec_type(), port_string_name() ) -> string().
get_port_description( _PortType=input_port_iteration, PortName ) ->
	text_utils:format( "input port iteration '~s'", [ PortName ] );

get_port_description( _PortType=output_port_iteration, PortName ) ->
	text_utils:format( "output port iteration '~s'", [ PortName ] );

get_port_description( _PortType=input_port, PortName ) ->
	text_utils:format( "input port '~s'", [ PortName ] );

get_port_description( _PortType=output_port, PortName ) ->
	text_utils:format( "output port '~s'", [ PortName ] ).



% Connects synchronously and directly (i.e. based on a direct request, not on an
% actor message - thus to be done initially) the specified output port of this
% dataflow block to the specified input port of the specified (downstream)
% dataflow block.
%
% (request)
%
...
connectOutputPortInitially( State, OutputPortBinName, DownstreamBlockPid,
	...
	?void_fmt( "Creating an initial channel from local output port ~w:~s to "
				"(downstream) input port ~w:~s.",
				[ self(), OutputPortBinName, DownstreamBlockPid,
				  InputPortBinName ] ),

	OutputPortTable = ?getAttr(output_ports),

	% Will be the source endpoint of the channel:
	OutputPort = case table:lookupEntry( OutputPortBinName, OutputPortTable ) of
		...
		key_not_found ->
			?error_fmt( "Initial connection request from (local) output port "
						"'~s' (to ~w:~s) rejected, as this port does not "
						"exist, knowing that ~s",
						[ OutputPortBinName, DownstreamBlockPid,
						  InputPortBinName,
	...
	OutputPortDescription = get_port_description( OutputPort ),
	...
	NewOutputPortTable = table:updateEntry( OutputPortBinName, NewOutputPort,
											OutputPortTable ),

	NewState = setAttribute( State, output_ports, NewOutputPortTable ),

	% Interleaved answer from requestConnectionToInputPortInitially:
	receive

		{ wooper_result, input_port_connected } ->

			PortStrings = [ text_utils:format( "~w:~s", [ B, P ] )
							|| { B, P } <- NewFedPorts ],

			?void_fmt( "Output port '~s' now feeding following ~B input "
						"port(s):~s", [ OutputPortBinName, length( PortStrings),
							 text_utils:strings_to_string( PortStrings ) ] ),

			?wooper_return_state_result( NewState, output_port_connected )
...
requestConnectionToInputPortInitially( State, InputPortBinName,
	...
	?void_fmt( "Received from process ~w a connection request from (upstream) "
				"output port ~w:~s to local input port '~s'; this upstream "
				"output port is a ~s",
				[ SenderPid, UpstreamBlockPid, OutputPortBinName,
				  InputPortBinName,
				  port_description_to_string( OutputPortDescription ) ] ),

	InputPortTable = ?getAttr(input_ports),

	NewInputPortTable = connect_to_input_port( InputPortBinName, InputPortTable,
			  UpstreamBlockPid, OutputPortBinName, OutputPortDescription,
			  _InitiatorInfo=SenderPid, State ),

	ConnectedState = setAttribute( State, input_ports, NewInputPortTable ),

	?wooper_return_state_result( ConnectedState, input_port_connected ).
...
connectOutputPort( State, OutputPortBinName,
				   { DownstreamBlockPid, InputPortBinName }, ActionId,
				   SendingActorPid )
  when is_binary( OutputPortBinName ) andalso is_pid( DownstreamBlockPid )
	...
	?void_fmt( "Connecting (local) output port '~s' to the (downstream) "
				"input port '~s' of block ~w, on behalf of actor ~w, "
				"for action #~B.",
	...
	OutputPort = case table:lookupEntry( OutputPortBinName, OutputPortTable ) of
		...
		key_not_found ->
			?error_fmt( "Connection request from (local) output port '~s' "
						"(to ~w:~s) rejected, as this port does not exist, "
						"knowing that ~s",
						[ OutputPortBinName, DownstreamBlockPid,
						  InputPortBinName,
						  list_output_ports( OutputPortTable ) ] ),
	...
	% directly notify the listener, without having to use this upstream
	% block as an intermediate):
	...
	?void_fmt( "Now this (local) output port '~s' is feeding following "
				"~B port identifiers:~s", [ OutputPortBinName,
				  length( NewFedPorts ),
...
-spec connectToDownstreamBlock( wooper:state(), [ canonical_connection_spec() ],
								downstream_block_pid(), action_id(),
								sending_actor_pid() ) -> actor_oneway_return().
connectToDownstreamBlock( State, ConnectionSpecs, DownstreamBlockPid, ActionId,
						  SendingActorPid ) ->

	?void_fmt( "Connecting to downstream block ~w, on behalf of actor ~w, "
				"for action #~B, based on ~s",
	...
	{ IterConnectionSpecs, IterState } = instantiate_output_iterated_ports(
	...
	% descriptions in a list of inbound_connection_spec() elements to be sent to
	% the downstream unit:
	%
	InboundConnectionSpec = get_inbound_connection_specs( IterConnectionSpecs,
														  IterState ),
	...
	?void_fmt( "Finished dealing with local ports verification and/or "
			   "instantiation; now requesting inbound connection to the "
			   "downstream block ~w with connection specifications ~p.",
			   [ DownstreamBlockPid, InboundConnectionSpec ] ),

	Oneway = { requestInboundConnections, [ InboundConnectionSpec,
											ListenerActorPid, ActionId ] },
...
instantiate_output_iterated_ports( _ConnectionSpecs=[
	...
	OutputIteration = case table:lookupEntry( OutputIterationBinName,
		...
		key_not_found ->
			?error_fmt( "Request to instantiate a non-existing output "
						"iteration '~s', whereas ~s", [ OutputIterationBinName,
	...
	?void_fmt( "Instantiated output port iteration '~s': new (output) "
				"iterated port '~s' created.",
				[ OutputIterationBinName, NewIteratedOutputPortName ] ),

	NewOutputIterationTable = table:updateEntry( OutputIterationBinName,
...
% Returns the inbound connection specs suitable for the downstream block, from
% the specified overall, more general connection specs.
%
% Checks that the specified local output ports exists, and gather their port
% description.
%
-spec get_inbound_connection_specs( [ canonical_connection_spec() ],
					wooper:state() ) -> [ inbound_connection_spec() ].
get_inbound_connection_specs( ConnectionSpecs, State ) ->

	OutputPortTable = ?getAttr(output_ports),

	get_inbound_connection_specs( ConnectionSpecs, _AccConnSpec=[],
								  OutputPortTable, State ).



get_inbound_connection_specs( _ConnectionSpecs=[], AccConnSpec,
							  _OutputPortTable, _State ) ->
	% Order does not matter:
	AccConnSpec;

get_inbound_connection_specs( _ConnectionSpecs=[
		  { { output_port_name, OutputPortBinName }, DownstreamPortSpec } | T ],
		AccConnSpec, OutputPortTable, State ) ->
	...
	OutputPort = case table:lookupEntry( OutputPortBinName, OutputPortTable ) of
		...
		key_not_found ->
			?error_fmt( "Connection request from (local) output port '~s' "
						"(to downstream ~s) rejected, as this local port "
						"does not exist, whereas ~s",
						[ OutputPortBinName,
	...
	% Forges a new inbound_connection_spec():
	NewInboundConnSpec = { OutputPortBinName, OutputPortDescription,
						   DownstreamPortSpec },

	get_inbound_connection_specs( T, [ NewInboundConnSpec | AccConnSpec ],
								  OutputPortTable, State ).
...
requestConnectionToInputPort( State, InputPortBinName, OutputPortBinName,
							  OutputPortDescription, ListenerActorPid, ActionId,
							  UpstreamBlockPid ) ->
	...
	?void_fmt( "Received from upstream block ~w a connection request from "
				"its '~s' output port to (local) input port '~s', in the "
				"context of action #~B; this "
				"upstream output port is a ~s", [ UpstreamBlockPid,
				OutputPortBinName, InputPortBinName, ActionId,
				port_description_to_string( OutputPortDescription ) ] ),
	...
	% Notified then the specified listener:
...
% (actor oneway)
%
-spec requestInboundConnections( wooper:state(), [ inbound_connection_spec() ],
	   actor_pid(), action_id(), sending_actor_pid() ) -> actor_oneway_return().
requestInboundConnections( State, InboundConnectionSpec, ListenerActorPid,
						   ActionId, UpstreamBlockPid ) ->

	% So we have in InboundConnectionSpec the description of the channels to
	% create. Let's apply this here, and notify back the caller.

	?void_fmt( "Received from upstream block ~w a request for ~B connections, "
			   "in the context of action #~B: ~p",
			   [ UpstreamBlockPid, length( InboundConnectionSpec ), ActionId,
	...
	{ PortPairs, NewInputPortTable, NewInputIterationTable } =
		apply_connections( UpstreamBlockPid, InboundConnectionSpec,
						   InputPortTable, InputIterationTable,
						   _AccPortPairs=[], State ),
	...
	FinalState = setAttributes( SentState, [
...
% (helper)
%
-spec apply_connections( upstream_block_pid(), [ inbound_connection_spec() ],
						 input_port_table(), input_iteration_table(),
						 [ port_pair() ], wooper:state() ) ->
	   { [ port_pair() ], input_port_table(), input_iteration_table() }.
apply_connections( _UpstreamBlockPid, _InboundConnectionSpec=[], InputPortTable,
				   InputIterationTable, AccPortPairs, _State ) ->
	{ AccPortPairs, InputPortTable, InputIterationTable };


% Targeting a standard input port here:
apply_connections( UpstreamBlockPid, _InboundConnectionSpec=[
	...
	?void_fmt( "Received from upstream block ~w a connection request from "
			   "its '~s' output port to (local) input port '~s'; this "
			   "upstream output port is a ~s", 
	...
	NewAccPortPairs = [ { OutputPortBinName, InputPortBinName }
						| AccPortPairs ],

	apply_connections( UpstreamBlockPid, T, NewInputPortTable,
					   InputIterationTable, NewAccPortPairs, State );
...
apply_connections( UpstreamBlockPid, _InboundConnectionSpec=[
	...
	?void_fmt( "Received from upstream block ~w a connection request from "
				"its '~s' output port to (local) input iteration '~s'; this "
				"upstream output port is a ~s", [ UpstreamBlockPid,
	...
	%
	InputIteration = case table:lookupEntry( InputIterationBinName,
		...
		key_not_found ->
			?error_fmt( "Request to connect a non-existing input iteration "
						"'~s', whereas ~s", [ InputIterationBinName,
	...
	?void_fmt( "Instantiated input port iteration '~s': new (input) "
			   "iterated port '~s' created.",
			   [ InputIterationBinName, NewIteratedInputPortName ] ),

	NewInputIterationTable = table:updateEntry( InputIterationBinName,
...
onInboundConnectionsCreated( State, PortPairs, ListenerActorPid,
	...
	?trace_fmt( "Received from downstream block ~w for action #~B "
				"following connection information (port pairs):~p",
				[ DownstreamBlockPid, ActionId, PortPairs ] ),
	...
	% down to record to which input ports each of them is now connected:
	%
	...
	%
	Oneway = { onConnectionsCreated, [ PortPairs, DownstreamBlockPid,
									   ActionId ] },
...
record_downstream_channels( DownstreamBlockPid,
	...
	OutputPort = case table:lookupEntry( OutputPortBinName, OutputPortTable ) of
		...
		key_not_found ->
			?error_fmt( "Connection notification from (local) output port '~s' "
						"(to ~w:~s) rejected, as this port does not exist, "
						"knowing that ~s.",
						[ OutputPortBinName, DownstreamBlockPid,
						  InputPortBinName,
	...
	?void_fmt( "Now (local) output port '~s' is connected to input port "
				"~w:~s.",
...
create_input_iterated_ports( CreationCount,
	...
	{ NewPortCount, _Bounds } = NewMultiplicity = update_multiplicity(
										  CreationCount, Multiplicity ),
	...
	NewInputPortTable = table:addNewEntries( NewPortEntries, InputPortTable ),
...
create_output_iterated_ports( CreationCount,
	...
	NewOutputPortTable = table:addNewEntries( NewPortEntries, OutputPortTable ),
...
get_iterated_port_name( IterationName, Index ) ->
	...
	PortName = text_utils:format( "~s~s~B",
...
% (anyway they are unlikely to reach extremely high values), and we always use
% fresh names.
%
% (helper)
%
...
insert_indexes( CreationCount, Indexes ) ->
	...
	RevNewIndexes = get_rev_new_indexes( CreationCount, FirstNewIndex,
										 _Acc=[] ),
...
requestOutputPortStatus( State, BinOutputPortName, ActorPid )
  when is_binary( BinOutputPortName ) ->
	...
	?void_fmt( "Received a request about the status of output port '~s' "
...
requestOutputPortStatus( State, OutputPortName, SenderPid )
  when is_list( OutputPortName ) ->
	...
	requestOutputPortStatus( State, BinOutputPortName, SenderPid ).
...
resume( State, _SenderPid ) ->

	% Checking:
	suspended = ?getAttr(run_status),

	?void_fmt( "Resuming now ~s.", [ to_string( State ) ] ),

	OutputPortTable = ?getAttr(output_ports),

	{ NewOutputPortTable, NewState } = send_suspended_values( OutputPortTable,
															  State ),
	...
	?wooper_return_state_only( ResumeState ).
...
send_suspended_values( _OutputPorts=[ { OutputPortName,
	...
	NewPortTable = table:addNewEntry( _K=OutputPortName, _V=NewOutputPort,
									  AccPortTable ),
...
send_suspended_values( _OutputPorts=[ { OutputPortName, OutputPort } | T ],
	...
	NewPortTable = table:addNewEntry( _K=OutputPortName, _V=OutputPort,
									  AccPortTable ),
...
disconnect_output_ports_from( BlockPid, OutputPortTable ) ->
	...
	NewOutputPorts = [ { PortName, disconnect_output_port( BlockPid,
														   OutputPort ) }
...
get_block_pid( ExternalId, State ) ->
	...
	?getAttr(identification_server_pid) ! { getBlockPID, [ ExternalId ],
											self() },
...
get_input_port( InputPortName, InputPortTable, State )
  when is_binary( InputPortName ) ->
	...
	case table:lookupEntry( _K=InputPortName, InputPortTable ) of
		...
		key_not_found ->
			...
			?error_fmt( "Error, specified input port named '~s' does "
						"not exist, knowing that ~s",
...
%
% Typically used for blocks:
%
%  - wanting that the corresponding input value remains as long as it is not
%  explicitly changed
%
%  - or relying on the 'activate_when_all_set activation' policy - knowing that
%  all input ports will be reset (i.e. put in 'unset') anyway
%
...
get_input_port_status( InputBinPortName, State )
  when is_binary( InputBinPortName ) ->
	...
	%?debug_fmt( "Reading for input port '~s' value status ~p.",
...
% Returns the (supposedly set) value of specified input port, and leaves it
% unchanged.
%
  ...
  when is_list( InputPortName ) ->
	...
	case InputPort#input_port.value_status of
		...
		{ set, InputPortValue } ->
			...
			UpdatedInputPortTable = table:updateEntry( BinPortName,
...
get_output_port( OutputPortName, OutputPortTable, State )
  when is_binary( OutputPortName ) ->
	...
	case table:lookupEntry( _K=OutputPortName, OutputPortTable ) of
		...
		key_not_found ->
			...
			?error_fmt( "Error, specified output port named '~s' does "
						"not exist, knowing that ~s", [ OutputPortName,
							  list_output_ports( OutputPortTable ) ] ),
...
get_output_port_status( BinOutputPortName, State )
  when is_binary( BinOutputPortName ) ->
	...
	%?debug_fmt( "Reading for output port '~s' value status ~p.",
...
% Returns the SUTC metadata associated with an output port.
%
% (helper)
%
...
get_input_port_iteration( InputIterationName, State )
  when is_binary( InputIterationName ) ->
	...
	case table:lookupEntry( InputIterationName, InputIterationTable ) of
		...
		key_not_found ->
			...
			?error_fmt( "Error, specified input port iteration '~s' does "
						"not exist, knowing that ~s.",
						[ InputIterationName,
						  list_input_iterations( InputIterationTable ) ] ),
...
get_output_port_iteration( OutputIterationName, State )
  when is_binary( OutputIterationName )->
	...
	case table:lookupEntry( OutputIterationName, OutputIterationTable ) of
		...
		key_not_found ->
			...
			?error_fmt( "Error, specified output port iteration '~s' does "
						"not exist, knowing that ~s.",
						[ OutputIterationName,
						  list_output_iterations( OutputIterationTable ) ] ),
...
% Returns the port description corresponding to specified port.
%
-spec get_port_description( input_port() | output_port() ) ->
								  port_description().
get_port_description( #output_port{
						 value_semantics=Semantics,
						 value_unit=Unit,
						 value_type=Type,
						 value_constraints=Constraints } ) ->
	% Does not depend on a port being input or output:
...
get_downstream_blocks( State ) ->
	...
	% Duplicates may exist:
	DownstreamBlockPids = [ DownstreamBlockPid ||
							  { DownstreamBlockPid, _InputPortName }
...
% Returns the set of the PID of all blocks directly connected to this block
% (whether they are downstream, upstream of both).
%
...
-spec validate_value_for_input_port( channel_value(), input_port(),
			input_port_name(), wooper:state() ) -> basic_utils:void().
validate_value_for_input_port( ChannelValue=#channel_value{
	...
	case rdf_utils:implies( ValueSemantics, PortSemantics ) of
		...
		false ->
			...
			?warning_fmt( "Trying to feed the input port '~s' with "
						  "a value (~s) whose semantics does not "
						  "comply with the one of this port, as the value  "
						  "lacks following ~B semantic elements: ~p.~n"
						  "Indeed the value relies on ~s~nwhile the port "
						  "relies on ~s."  ,
	...
	case unit_utils:are_units_identical( ValueUnit, PortUnit ) of
		...
		false ->
			...
			?error_fmt( "Trying to feed the input port '~s' with a value (~s) "
						"whose unit (~s) does not match the one of "
						"this port (~s).",
						[ PortName, value_to_string( ChannelValue ),
	...
	%
	case meta_utils:are_types_identical( ValueType, PortType ) of
		...
		false ->
			?error_fmt( "Trying to feed the input port '~s' with a value (~s) "
						"whose type (~s) does not match the one of this port "
						"(~s).",
						[ PortName, value_to_string( ChannelValue ),
						  meta_utils:type_to_string( ValueType ),
						  meta_utils:type_to_string( PortType ) ] ),
	...
	case satisfies_constraints( Value, PortConstraints ) of
		...
		{ false, Constraint } ->
			?error_fmt( "Trying to feed the input port '~s' with a value (~s) "
						"that violates the following constraint of that "
						"port: ~w.",
						[ PortName, value_to_string( ChannelValue ),
						  Constraint ] ),
...
-spec validate_value_for_output_port( channel_value(), output_port(),
			output_port_name(), wooper:state() ) -> basic_utils:void().
validate_value_for_output_port( ChannelValue=#channel_value{
	...
	case rdf_utils:implies( ValueSemantics, PortSemantics ) of
		...
		false ->
			...
			?warning_fmt( "Trying to feed the output port '~s' with "
						  "a value (~s) whose semantics does not "
						  "comply with the one of this port, as the value  "
						  "lacks following ~B semantic elements: ~p.~n"
						  "Indeed the value relies on ~s~nwhile the port "
	...
	case unit_utils:are_units_identical( ValueUnit, PortUnit ) of
		...
		false ->
			ValueUnitString = text_utils:binary_to_string( ValueUnitBinString ),
			PortUnitString = text_utils:binary_to_string( PortUnitBinString ),
			?error_fmt( "Trying to feed the output port '~s' with a value (~s) "
						"whose unit (~s) does not match the one of "
						"this port (~s).",
						[ PortName, value_to_string( ChannelValue ),
						  ValueUnitString, PortUnitString ] ),
	...
	case satisfies_constraints( Value, PortConstraints ) of
		...
		{ false, Constraint } ->
			?error_fmt( "Trying to feed the output port '~s' "
						"with a value (~s) that violates the following "
						"constraint of that port: ~w.",
						[ PortName, value_to_string( ChannelValue ),
						  Constraint ] ),
			throw( { unsatisfied_constraint, Constraint, Value } )
...
assign_input_value( ChannelValue=#channel_value{ actual_value=Value },
					InputPort=#input_port{ value_type=ExpectedType },
					BinPortName, State ) ->

	case meta_utils:is_of_type( Value, ExpectedType ) of
		...
		false ->
			DetectedType = meta_utils:get_type_of( Value ),

			%io:format( "Is value '~p' of type '~p'? No, of type '~p'.~n",
			%		   [ Value, ExpectedType, DetectedType ] ),

			?error_fmt( "The value '~p' (corresponding to ~s) is determined "
						"as being of type '~s' whereas the expected type was "
						"'~s' for ~s",
						[ Value, value_to_string( ChannelValue ),
						  meta_utils:type_to_string( DetectedType ),
						  meta_utils:type_to_string( ExpectedType ),
	...
	case meta_utils:is_of_type( Value, ExpectedType ) of
		...
		true ->
			...
			case ?getAttr(run_status) of
				...
				active ->
					PortTimestamp = class_Actor:get_current_logical_timestamp(
									  State ),
					...
					SentState = notify_fed_input_ports( ChannelValue, FedPorts,
														State ),
...
notify_fed_input_ports( ChannelValue,
	...
	?void_fmt( "Feeding the input port '~s' of downstream block ~w with "
				"value ~s.", [ InputPortName, DownstreamBlockPid,
...
get_all_input_iteration_values( IterationName, State )
  when is_list( IterationName ) ->

	BinIterationName = text_utils:string_to_binary( IterationName ),

	Iteration = #input_port_iteration{ base_name=BaseName,
									   port_indexes=Indexes }
		= get_input_port_iteration( BinIterationName, State ),

	?void_fmt( "Getting all values from ~s.",
	...
	% Returns ChannelValues, or raises badmatch if ever a port was not set:
...
set_input_port_values_helper( _PortPairs=[ { PortName, ChannelValue } | T ],
	...
	NewInputPortTable = table:updateEntry( BinPortName, NewInputPort,
										   InputPortTable ),
...
set_input_port_direct_values_helper(
  _PortPairs=[ { InputPortName, DirectValue } | T ], InputPortTable, State ) ->
	...
	% the actual value that it contain, we only create here a (bogus) empty
	...
	NewInputPortTable = table:updateEntry( InputPortBinName, NewInputPort,
										   InputPortTable ),
...
% (helper)
%
-spec list_input_ports( input_port_table() ) -> string().
list_input_ports( InputPortTable ) ->
	...
	case table:keys( InputPortTable ) of
		...
		PortNames ->
			SortedNames = lists:sort( PortNames ),
			ListedNames = text_utils:binaries_to_string( SortedNames ),
			text_utils:format( "the ~B existing input ports are "
							   "(alphabetically):~s",
							   [ length( PortNames ), ListedNames ] )
...
% (helper)
%
-spec list_output_ports( output_port_table() ) -> string().
list_output_ports( OutputPortTable ) ->
...
% (helper)
%
-spec list_input_iterations( input_iteration_table() ) -> string().
list_input_iterations( InputIterationTable ) ->
	...
	case table:keys( InputIterationTable ) of
		...
		IterationNames ->
			SortedNames = lists:sort( IterationNames ),
			ListedNames = text_utils:binaries_to_string( SortedNames ),
			text_utils:format( "the ~B existing input iterations are "
							   "(alphabetically):~s",
							   [ length( IterationNames ), ListedNames ] )
...
% (helper)
%
-spec list_output_iterations( output_iteration_table() ) -> string().
list_output_iterations( OutputIterationTable ) ->
...
set_output_port_value( OutputPortName, ChannelValue, State ) ->
	...
	?void_fmt( "Setting output port '~s' to ~s",
...
set_output_ports_internal( OutputPortBinName, ChannelValue, OutputPortTable,
  ...
  when is_binary( OutputPortBinName ) ->
	...
	OutputPort = case table:lookupEntry( OutputPortBinName, OutputPortTable ) of
		...
		key_not_found ->
			?error_fmt( "Output port '~s' not found; ~s",
						[ OutputPortBinName,
						  list_output_ports( OutputPortTable ) ] ),
	...
	NewPortTable = table:updateEntry( OutputPortBinName, NewOutputPort,
									  OutputPortTable ),
...
% Sets the specified output port to the specified direct (i.e. plain,
% non-channel) values.
%
% Typically useful at times like initialisation, where direct values (with no
% associated metadata), are available, and must be assigned to ports.
...
set_output_port_direct_values_helper(
  _PortPairs=[ { PortName, DirectValue } | T ], OutputPortTable, State ) ->
	...
	#output_port{ value_semantics=Semantics,
				  value_unit=Unit,
				  value_type=Type } = get_output_port( BinPortName,
													   OutputPortTable, State ),
	...
	% (ex: regarding types) would then have be performed.
	%
	OutputPort = table:getEntry( BinPortName, OutputPortTable ),
	...
	NewOutputPortTable = table:updateEntry( BinPortName, NewOutputPort,
											OutputPortTable ),
...
-spec declare_static_information_for(
		[ block_type() | class_DataflowUnitManager:managed_unit_spec() ] ) ->
...
declare_static_information_for( DataflowBlockTypes ) ->
	...
	declare_static_information_for( DataflowBlockTypes, SemanticServerPid,
									TypeServerPid ).


% (helper)
%
-spec declare_static_information_for( [ block_type() |
								class_DataflowUnitManager:managed_unit_spec() ],
...
%
-spec get_and_trigger_semantics( block_type() |
								 class_DataflowUnitManager:managed_unit_spec(),
								 semantic_server_pid() ) -> boolean().
...
% Returns whether an acknowledgement is waited for.
%
-spec get_and_trigger_types( block_type() |
							 class_DataflowUnitManager:managed_unit_spec(),
							 type_server_pid() ) -> boolean().
...
% (helper)
%
-spec get_referenced_semantics( [ input_port_spec() ], [ output_port_spec() ] )
...
get_referenced_semantics( InputPortSpecs, OutputPortSpecs ) ->
	...
	list_utils:uniquify( InputSemantics ++ OutputSemantics ).
...
get_referenced_types( InputPortSpecs, OutputPortSpecs ) ->
	...
	list_utils:uniquify( InputTypes ++ OutputTypes ).
...
% (helper)
%
-spec get_declared_semantics( block_type() |
							  class_DataflowUnitManager:managed_unit_spec() ) ->
			'no_semantics_declared' | class_SemanticServer:vocabulary().
...
get_declared_semantics( { DataflowBlockClassname, Language } ) ->
	...
	case meta_utils:is_function_exported( ActualClassname, SemFunName,
										  _Arity=1 ) of

		true ->
			apply( ActualClassname, SemFunName,
				   _SemArgs=[ DataflowBlockClassname ] );
...
get_declared_semantics( DataflowBlockClassname ) ->
	...
	case meta_utils:is_function_exported( DataflowBlockClassname,
		...
		true ->
			apply( DataflowBlockClassname, SemFunName, _SemArgs=[] );
...
% (helper)
%
-spec get_declared_types( block_type() |
						  class_DataflowUnitManager:managed_unit_spec() ) ->
								'no_types_declared' |
...
get_declared_types( { DataflowBlockClassname, Language } ) ->
	...
	case meta_utils:is_function_exported( ActualClassname, TypeFunName,
										  _Arity=1 ) of

		true ->
			apply( ActualClassname, TypeFunName,
				   _TypeArgs=[ DataflowBlockClassname ] );
...
get_declared_types( DataflowBlockClassname ) ->
	...
	case meta_utils:is_function_exported( DataflowBlockClassname,
		...
		true ->
			apply( DataflowBlockClassname, TypeFunName, _TypeArgs=[] );
...
-spec get_static_information( wooper:state() ) ->
		{ class_SemanticServer:vocabulary(), class_TypeServer:type_entries() }.
get_static_information( State ) ->

	BasicClassname = wooper:get_class_name( State ),

	ActualUnitSpec = case BasicClassname of

		class_DataflowPythonProcessingUnit ->
			WooperPythonClass = wooper_utils:pep8_class_to_wooper_class(
	...
	get_static_information( State, ActualUnitSpec ).
...
-spec get_static_information( wooper:state(),
							  class_DataflowUnitManager:managed_unit_spec() ) ->
									{ class_SemanticServer:vocabulary(),
									  class_TypeServer:type_entries() }.
get_static_information( State, UnitSpec ) ->

	Vocabulary = case get_declared_semantics( UnitSpec ) of

		no_semantics_declared ->
			% Warning would be overkill, yet...
			?warning( "This dataflow block type does not declare its "
					  "semantics." ),
			[];
	...
	TypeEntries = case get_declared_types( UnitSpec ) of
		...
		no_types_declared ->
			% Warning would be overkill, yet...
			?warning( "This dataflow block type does not declare its typing "
					  "information." ),
			[];
...
% (exported helper, for convenience)
%
-spec get_port_specifications( block_type() |
							class_DataflowUnitManager:managed_unit_spec() ) ->
	'no_port_specifications_declared' |
...
get_port_specifications( { DataflowBlockClassname, Language } ) ->
	...
	case meta_utils:is_function_exported( ActualClassname, PortFunctionName,
		...
		true ->
			apply( ActualClassname, PortFunctionName,
				   _Args=[ DataflowBlockClassname ] );
...
get_port_specifications( DataflowBlockClassname ) ->
	...
	case meta_utils:is_function_exported( DataflowBlockClassname,
		...
		true ->
			apply( DataflowBlockClassname, PortFunctionName, _Args=[] );
...
createOutputPorts( State, OutputPortSpecs ) ->
	...
	?wooper_return_state_result( OutputState, output_ports_created ).
