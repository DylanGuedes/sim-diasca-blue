...
class Block(TraceEmitter):
    ...
    def get_input_port_value(self, input_port_name: PortName):
        """Returns the value currently held by the specified, supposedly set,
        standard input port.
        """
    ...
    # Sets a channel value on an output port identified by its name and encodes
    # this name at the same time for a further sending to Erlang:
    ...
    def set_all_output_iteration_values(self, iteration_name: PortName,
        ...
        if port_count == 0:
            ...
            # created, because the model is not itself connected to downstream),
            # writings will be nevertheless tolerated:

            msg = "trying to feed the output port iteration named '{}' " \
                  "with {} values whereas it is not connected (writing " \
                  "thus ignored as a whole).".format(iteration_name,
                                                     number_of_values)
            ...
            if number_of_values < port_count:
                self.send_error("Trying to feed the {} iterated ports of "
