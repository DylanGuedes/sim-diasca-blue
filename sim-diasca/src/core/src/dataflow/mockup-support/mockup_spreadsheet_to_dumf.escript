...
get_usage() ->
	io:format( "Usage: mockup_spreadsheet_to_dumf.escript my_file.[xlsx|ods]~n"
			   ...
			   "MyUnit.ods to MyUnit.dumf).~nPlease refer to the 'SimDiasca "
...
unzip_xlsx_file( FileName, ExpectedSheetTitles, DebugMode ) ->
	...
	% Finds the sheet files which titles match an expected one:
	WorksheetBaseNames = extract_sheets_from_workbook( Workbook,
													   ExpectedSheetTitles ),
...
main_ods( FileName, ExpectedSheetTitles, DebugMode ) ->
	...
	{ TmpDir, ContentFile, DUMFFileName } = unzip_ods_file( FileName,
															DebugMode ),

	% Reads the content file and generates and generates raw arrays listing the
	% contents of each sheet in a { Title, Array } format, where each array is
...
% whose 'name' attribute is matching. If successful, returns the associated file
% name deduced from the rank stored in the 'r:id' attribute:
%
...
identify_sheet_from_title( [ SheetElement | T ], Title ) ->
	...
	case string:equal( SheetName, Title ) of
		...
		% If matched, returns the file name (which has the form "sheet" ++ Id ++
...
parse_row_cells( [ RowCell | T ], SharedStrings, RowIndex, ColumnIndex, Acc,
	...
	% Checks the 't' attribute which is either not defined or whose value is
	% 's', depending on the original type of the cell value:
	%
	CellValue = case get_attribute_value( 't', RowCell ) of
		...
		%
		SimpleAttr when SimpleAttr =:= undefined orelse
						SimpleAttr =:= "str" ->
	...
	% Creates a new array cell aggregated to the previously formed ones and goes
	% to the next cells:
	%
...
parse_ods_row_cells( [ Cell | T ], RowIndex, ColumnIndex, Acc, DebugMode )
  when Cell#xmlElement.content == [] ->
	...
	% giving the number of empty colums that follow the cell:
	%
...
%
parse_raw_data_arrays( SheetsData, Titles, DebugMode ) ->

	% Extracts every title from the list:
	[ MetadataTitle, InputPortSpecsTitle, OutputPortSpecsTitle,
	  MockupClausesTitle ] = Titles,
...
extract_metadata( RawMetadata ) ->
	...
	% number and which is located outside the main array:
...
format_input_port_specs( [ InputPortSpec | T ], Acc ) ->
	...
	% Extracts the pieces of input port specifications and checks they are valid
	% strings:
...
%
format_output_port_specs( OutputPortSpecs ) ->
	ReversedFormattedOPSs = format_output_port_specs( OutputPortSpecs,
													  _Acc=[] ),
...
format_input_match_specs( [ _InputMatchSpec = [ PortName, AtomStr ]
							| MoreSpecs ], IPTypes, Acc ) ->

	CoreFormattedIMS = io_lib:format( "~c~c{ ~p, ~p }",
								  [ 9, 9, PortName, list_to_atom( AtomStr ) ] ),

	FormattedIMS = format_and_append_if_no_tail( ",~n", CoreFormattedIMS,
												 MoreSpecs ),

	NewAcc = [ FormattedIMS | Acc ],
	format_input_match_specs( MoreSpecs, IPTypes, NewAcc );


format_input_match_specs( [ _InputMatchSpec = [ PortName, "set", ValueStr ]
							| MoreSpecs ], IPTypes, Acc ) ->
...
format_input_match_specs( [ _InputMatchSpec = [ PortName, "around", ValueStr ]
							| MoreSpecs ], IPTypes, Acc ) ->
...
format_input_match_specs( [ _InputMatchSpec = [ PortName, "around", ValueStr1,
												ValueStr2 ] | MoreSpecs ],
						  IPTypes, Acc ) ->
...
format_input_match_specs( [ _InputMatchSpec = [ PortName, "between", ValueStr1,
												ValueStr2 ] | MoreSpecs ],
						  IPTypes, Acc ) ->
...
format_input_match_specs( [ InputMatchSpec = [ PortName, "among", ValListStr ]
							| MoreSpecs ], IPTypes, Acc ) ->
	...
	ValuesList = case get_list_from_string( ValListStr ) of
		...
		[] ->
			io:format( "~cError: an empty 'among' specification has been "
					   ...
					   "Please use the 'unset' specification rather or refer "
	...
	CoreFormattedIMS = io_lib:format( "~c~c{ ~p, { ~p, ~p } }", [ 9, 9,
												PortName, among, ValuesList ] ),
...
format_output_state_specs( OutputStateSpecs, OPTypes ) ->
	ReversedSpecList = format_output_state_specs( OutputStateSpecs, OPTypes,
												  _Acc=[] ),
	lists:reverse( ReversedSpecList ).
...
format_output_state_specs( [ _OutputStateSpec = [ PortName, AtomStr ]
							 | MoreSpecs ], OPTypes, Acc ) ->

	CoreFormattedOSS = io_lib:format( "~c~c{ ~p, ~p }",
								  [ 9, 9, PortName, list_to_atom( AtomStr ) ] ),

	FormattedOSS = format_and_append_if_no_tail( ",~n", CoreFormattedOSS,
												 MoreSpecs ),

	NewAcc = [ FormattedOSS | Acc ],

	format_output_state_specs( MoreSpecs, OPTypes, NewAcc );


format_output_state_specs( [ _OutputStateSpec = [ PortName, "set", ValueStr ]
							 | MoreSpecs ], OPTypes, Acc ) ->
...
format_output_state_specs( [ _OutputStateSpec = [ PortName, "state_of", IPName ]
							 | MoreSpecs ], OPTypes, Acc ) ->
...
get_cell_rank_from_value( Value, [ Cell | MoreCells ], Rank ) ->
	...
	case Cell of
		...
		_AnythingElse ->
			get_cell_rank_from_value( Value, MoreCells, Rank+1 )
...
fill_empty_cells( RowCells, NumberOfColumns ) ->
	fill_empty_cells( RowCells, NumberOfColumns, 0, [] ).

% Algo final case: once ExpectedColIndex reaches NumberOfColumns, a full row has
% been aggregated in Acc. Then the extra cells (if any) are returned separately
% to be joined back inside the big unsplit array.
...
% Algo loop case 1: if the current cell contains the column index currently
% looked for (ExpectedColIndex in other clauses), it can be passed to Acc. We
% will now look for the next column index, in the next cells (MoreCells).
fill_empty_cells( [ Cell = { _RowIndex, ColIndex, _Value } | MoreCells ],
				  NumberOfColumns, ColIndex, Acc ) ->
	fill_empty_cells( MoreCells, NumberOfColumns, ColIndex+1, [ Cell | Acc ] );

% Algo loop case 2: if the current cell does not contain the column index
...
fill_empty_cells( Cells = [ { RowIndex, _ColIndex, _Value } | _MoreCells ],
...
% Defines some default values which meanings are coherent with the ones of each
% column:
%
% Port name
...
% Displays a text message according a boolean value.
%
...
% Displays a formatted text message according a boolean value.
%
...
%
-spec check_file_exists( string(), boolean() ) -> ok.
...
%
-spec remove_directory( string() ) -> ok.
...
% Taken verbatim from common/src/utils/file_utils.erl:
...
% End of verbatim section for common/src/utils/file_utils.erl:



% Taken verbatim from common/src/utils/text_utils.erl:
...
% End of verbatim section for common/src/utils/text_utils.erl:
