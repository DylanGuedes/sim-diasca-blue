...
run() ->
	...
	?test_info_fmt( "Initial status of server: ~s", [ InitialStatusString ] ),
	...
	?test_info_fmt( "Intermediate status of server: ~s",
					[ IntermediateStatusString ] ),



	% To have a different (quickly dead) PID:
	SecondBlockPid = spawn( fun() -> ok end ),
	...
	?test_info_fmt( "Final status of server: ~s", [ FinalStatusString ] ),
