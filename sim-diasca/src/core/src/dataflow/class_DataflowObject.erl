...
%
% Dataflow objects may be direct instances of the current class (with then no
% specific overriding needed).
%
...
-define( wooper_construct_parameters, ObjectSettings, DataflowObjectName,
		 DataflowAttributeSpecs, InitialAttributeValues,
		 SpecForUniquePeers, SpecForMultiplePeers, DataflowPid ).
...
%
-define( wooper_method_export, onFirstDiasca/2, updateAttributes/4,
		 triggerDestruction/3, registerPeerAs/4, unregisterPeer/4,
		 getUniquePeerFor/2, getMultiplePeersFor/2,
		 notifyNewInput/4 ).


% Convenience peer-related exports:
-export([ register_peer/3, unregister_peer/3,
		  get_unique_peer_for/2, get_multiple_peers_for/2 ]).
...
% Each attribute of a dataflow object is thus embodied by two ports: an input
% one (to set it), and an output one (to notify downstream blocks of a new
% value); no other port is to be defined.
%
%
% When a dataflow object is constructed, it starts in suspended mode; otherwise,
% not being connected yet, assigning its output ports at creation time to their
% respective initial values would not trigger any downstream block.
%
...
% so that their initial values may propagate directly during the leading
% dataflow evaluation.
...
%
-type unique_peer_table() :: table:table( peer_type(),
										  basic_utils:maybe( peer_pid() ) ).
...
% multiple peer types.
%
-type multiple_peer_table() :: table:table( peer_type(), [ peer_pid() ] ).
...
%
-type peer_to_name_table() :: table:table( peer_type(), peer_attribute_name() ).
...
% A dimension is a collection of symbols specialising a logicial attribute into
...
%
% Each of the dataflow attribute leads to the creation of two associated ports:
% an input port to set it, an output port to read it.




% Attributes specific to dataflow objects are:
%
% - attribute_specs :: [ dataflow_attribute_spec() ] is a list of the attribute
% specificationss used to create this dataflow object; useful whenever having to
% set a proper channel value to a given attribute
%
% - unique_peer_table :: unique_peer_table() is a table that associates, to
% each (single) peer associated to a unique peer type, its value (if any)
%
% - multiple_peer_table :: multiple_peer_table() is a table that associates,
% to each peer associated to a multiple peer type, its value (if any)
%
% - peer_to_name_table :: peer_to_attribute_table() is a table allowing to
% determine, for a given (unique) peer, the associated dataflow attribute name
% publishing its external identifier
%
% - accept_multiple_valued_attrs :: boolean() tells whether having more than one
% initial value defined per attribute is accepted (generally set to false)
%
% - accept_non_valued_attrs :: boolean() tells whether having no initial value
% defined for an attribute is accepted; generally set to true, as no initial
% values are usually set of output ports
%
% - accept_extra_attr_values :: boolean() tells whether initial values provided
% for non-existing dataflow attributes are allowed
%
% - identification_server_pid :: basic_utils:maybe( identification_server_pid()
% ) is the PID of the identification server (if any)
...
%
% - InitialAttributeValues is a list of the initial, direct values for each of
% the previously defined dataflow attributes, in the same order
%
...
%
% - SpecForMultiplePeers contains a list of atoms for which one might
% later define multiple links with other objects; typically one might pass
% [ class_Building] to indicate this object can contain several Buildings
%
...
-spec construct( wooper:state(), class_Actor:actor_settings(), object_name(),
				 [ dataflow_attribute_spec() ], [ actual_value() ],
				 unique_peer_spec(), multiple_peer_spec(), dataflow_pid() ) ->
					   wooper:state().
...
destruct( State ) ->
	...
	case [ UniquePair || UniquePair={ _PeerType, PeerPid } <- UniquePeerPairs,
		...
		StillUniques ->
			UniqueStrings = [ text_utils:format( "peer type '~s' to ~w",
				   [ Type, Pid ] ) || { Type, Pid } <- StillUniques ],

			?error_fmt( "At destruction, still ~B unique peers associated:~s",
	...
	case [ MultiplePair || MultiplePair={ _PeerType, PeerPidList }
		...
		StillMultiples ->
			MultipleStrings = [ text_utils:format( "peer type '~s' to ~w",
				   [ Type, Pids ] ) || { Type, Pids } <- StillMultiples ],

			?error_fmt( "At destruction, still ~B multiple peers associated:~s",
						[ length( MultipleStrings ),
	...
	case table:enumerate( ?getAttr(peer_to_name_table) ) of
		...
		PeerNames ->
			NameStrings = [ text_utils:format(
							  "peer ~w associated to attribute '~s'",
							  [ Peer, AttrName ] )
							|| { Peer, AttrName } <- PeerNames ],

			?error_fmt( "At destruction, still ~B associations between "
						"(unique) peer types and dataflow attributes:~s",
						[ length( PeerNames ),
...
%
-spec create_attribute_specs_for_peer( dataflow_attribute_name(),
   meta_utils:type_name(), peer_multiplicity() ) -> dataflow_attribute_spec().
create_attribute_specs_for_peer( PeerName, _Typename=string,
...
get_multiple_peer_table( _SpecForMultiplePeers=[ { PeerType, AttrName } | T ],
						 Acc ) when is_atom( PeerType ) andalso
									is_list( AttrName) ->
	PeerEntry = { PeerType, [] },
...
get_port_specs_for( _DataflowAttributeSpecs=[ #dataflow_attribute_spec{
	...
	NewOutputSpec = #output_port_spec{
					  name=AttrName,
					  comment=Comment,
					  is_iteration=false,
...
% update time): then output ports set their timestamp to 'send_on_resume' rather
% than propagating directly their values.
%
...
assign_channel_value_to_attribute( ChannelValue, BinAttrName, InputPortTable,
	...
	NewInputPortTable = table:addEntry( BinAttrName, NewInputPort,
										InputPortTable ),
	...
	NewOutputPortTable = table:addEntry( BinAttrName, NewOutputPort,
										 OutputPortTable ),
...
-spec create_channel_value_for_attribute( actual_value(),
							  dataflow_attribute_spec() ) -> channel_value().
create_channel_value_for_attribute( ActualValue, #dataflow_attribute_spec{
									  semantics=Semantics,
									  unit=Unit,
									  type_description=TypeDescription } ) ->
	class_Dataflow:create_channel_value( ActualValue, Semantics, Unit,
										 TypeDescription ).
...
-spec onFirstDiasca( wooper:state(), sending_actor_pid() ) ->
						   actor_oneway_return().
onFirstDiasca( State, _SenderPid ) ->
...
registerPeerAs( State, PeerPid, PeerType, _SenderPid ) ->
...
register_peer( PeerPid, PeerType, State ) when is_pid( PeerPid ) ->
	...
	%?trace_fmt( "Request to register a peer of type '~s': ~w.",
	%			[ PeerType, PeerPid ] ),
	...
	case table:lookupEntry( PeerType, MultiplePeerTypeTable ) of
		...
		{ value, PeerPidList } ->
			...
			case lists:member( PeerPid, PeerPidList ) of
				...
				true ->
					?debug_fmt( "The ~w '~s' peer instance belongs to the "
				...
				false ->
					?debug_fmt( "Registering ~w among the '~s' peers.",
								[ PeerPid, PeerType ] ),

					% Update by design:
					NewTable = table:addEntry( PeerType,
											   [ PeerPid | PeerPidList ],
		...
		key_not_found ->
			...
			case table:lookupEntry( PeerType, UniquePeerTable ) of
				...
				{ value, AlreadySetPeerPid } ->
					?error_fmt( "Cannot register peer ~w as a unique '~s' "
				...
				key_not_found ->
					...
					?error_fmt( "Cannot register ~w as a '~s' peer, as "
								"this peer type is neither declared as a "
								"unique or multiple one. "
								"Unique peer types are:~s~n"
								"Multiple peer types are:~s",
...
link_with_unique( PeerType, PeerPid, UniquePeerTable, State ) ->
	...
	?debug_fmt( "Linking with a unique '~s' peer: ~w.", [ PeerType, PeerPid ] ),

	% Update by design:
	NewPeerTable = table:addEntry( PeerType, PeerPid, UniquePeerTable ),
	...
	case table:lookupEntry( PeerType, PeerToNameTable ) of

		{ value, BinAttrName } ->

			PeerId = get_external_id_for( PeerPid, State ),

			?debug_fmt( "Setting the dataflow attribute '~s' to the external "
						"identifier of '~s' (unique) peer ~w, ~p.",
...
-spec unregisterPeer( wooper:state(), peer_type(), peer_pid(),
					sending_actor_pid() ) -> actor_oneway_return().
unregisterPeer( State, PeerPid, PeerType, _SenderPid ) ->
...
unregister_peer( PeerPid, PeerType, State ) when is_pid( PeerPid ) ->
	...
	%?trace_fmt( "Request to unregister a peer of type '~s': ~w.",
	%			[ PeerType, PeerPid ] ),
			...
			case list_utils:delete_if_existing( PeerPid, PeerPidList ) of
				...
				not_found ->
					?error_fmt( "Attempt to unregister peer ~w from the "
								"(multiple) peer type '~s' failed, as it does "
				...
				ShrunkPidList ->
					?debug_fmt( "Unregistering peer ~w from the multiple "
								"peer type '~s'.", [ PeerPid, PeerType ] ),

					% Update by design:
					NewTable = table:addEntry( PeerType, ShrunkPidList,
											   MultiplePeerTypeTable ),
		...
		key_not_found ->
			...
			case table:lookupEntry( PeerType, UniquePeerTable ) of
				...
				{ value, OtherMaybePeerPid } ->
					?error_fmt( "Cannot unregister peer ~w from unique "
								"peer type '~s', as it is not registered "
								"(current peer: ~w).",
				...
				key_not_found ->
					...
					?error_fmt( "Cannot unregister peer ~w from peer type '~s',"
								" as this peer type is neither declared as a "
								"unique or multiple one. "
								"Unique peer types are:~s~n"
								"Multiple peer types are:~s",
								[ PeerPid, PeerType, UniqueStr, MultipleStr ] ),
...
unlink_from_unique( PeerType, PeerPid, UniquePeerTable, State ) ->
	...
	?debug_fmt( "Unlinking from a unique '~s' peer: ~w.",
				[ PeerType, PeerPid ] ),

	% Update by design:
	NewPeerTable = table:addEntry( PeerType, undefined, UniquePeerTable ),
	...
	case table:lookupEntry( PeerType, PeerToNameTable ) of
		...
		{ value, BinAttrName } ->
			...
			?debug_fmt( "Unsetting the dataflow attribute '~s' corresponding "
						"to '~s' (unique) peer ~w.",
...
getUniquePeerFor( State, PeerType ) ->
	...
	?wooper_return_state_result( State, MaybePeerPid ).
...
get_unique_peer_for( PeerType, State ) ->
	...
	case table:lookupEntry( PeerType, ?getAttr(unique_peer_table) ) of
...
% (const request)
%
-spec getMultiplePeersFor( wooper:state(), peer_type() ) -> [ peer_pid() ].
getMultiplePeersFor( State, PeerType ) ->

	PeerPidList = get_multiple_peers_for( PeerType, State ),

	?wooper_return_state_result( State, PeerPidList ).
...
get_multiple_peers_for( PeerType, State ) ->
	...
	case table:lookupEntry( PeerType, ?getAttr(multiple_peer_table) ) of
...
-spec updateAttributes( wooper:state(), [ attribute_update() ], event_id(),
						sending_actor_pid() ) -> actor_oneway_return().
updateAttributes( State, AttributeUpdates, EventId, SenderPid ) ->

	?debug_fmt( "Updating following ~B attributes:~s",
				[ length( AttributeUpdates ),
				  text_utils:strings_to_string(
					[ attribute_update_to_string( N, V ) ||
...
-spec triggerDestruction( wooper:state(), event_id(), sending_actor_pid() ) ->
								actor_oneway_return().
triggerDestruction( State, EventId, SenderPid ) ->
...
notifyNewInput( State, InputPortName, ChannelValue, UpstreamBlockPid )
  when is_binary( InputPortName )
	...
	?debug_fmt( "Dataflow-based setting of attribute '~s' "
				"(through the input port of the same name) to the value '~s' "
	...
	?wooper_return_state_only( AssignState );
...
% name.
%
-spec get_attribute_spec_from_name( dataflow_attribute_name(), wooper:state() )
								  -> dataflow_attribute_spec().
	...
	case lists:keyfind( _K=AttributeName, _N=2, AttrSpecs ) of
		false ->
			...
			?error_fmt( "No attribute specification found for attribute "
						"'~s'; known attribute specifications:~s",
						[ AttributeName,
...
%
-spec attribute_spec_to_string( dataflow_attribute_spec() ) -> string().
...
attribute_spec_to_string( #dataflow_attribute_spec{
	...
	text_utils:format( "dataflow attribute named '~s', ~s, ~s, ~s, ~s and ~s",
...
to_string( State ) ->
	...
	AttrString = text_utils:format(
				   "~B attribute specifications were defined:~s",
				   [ length( AttrSpecs ),
					 text_utils:strings_to_string( AttrStrings ) ] ),
	...
	text_utils:format( "Dataflow Object named '~s', being ~s, "
					   "having ~s and ~s; ~s; ~s; ~s and ~s",
					   [ ?getAttr(name), ?getAttr(run_status),
						 InputDetailed, OutputDetailed, AttrString,
						 UniqueString, MultipleString, NameString ] ).



% Returns a textual representation of specified unique peer table.
%
-spec unique_peer_table_to_string( unique_peer_table() ) -> string().
unique_peer_table_to_string( Table ) ->
	...
	case table:enumerate( Table ) of
		...
		Entries ->
			...
			text_utils:format( "~B unique peers defined: ~s",
							   [ length( Entries ),
...
%
-spec unique_peer_type_to_string( peer_type(),
								  basic_utils:maybe( peer_pid() ) ) -> string().
unique_peer_type_to_string( PeerType, undefined ) ->
	text_utils:format( "peer type '~s' not associated to any peer",
					   [ PeerType ] );

unique_peer_type_to_string( PeerType, PeerPid ) ->
	text_utils:format( "peer type '~s' associated to peer ~w",
					   [ PeerType, PeerPid ] ).
...
%
-spec multiple_peer_table_to_string( multiple_peer_table() ) -> string().
multiple_peer_table_to_string( Table ) ->
	...
	case table:enumerate( Table ) of
		...
		Entries ->
			...
			text_utils:format( "~B multiple peers defined: ~s",
							   [ length( Entries ),
								 text_utils:strings_to_string( Strings ) ] )
...
%
-spec multiple_peer_type_to_string( peer_type(), [ peer_pid() ] ) -> string().
multiple_peer_type_to_string( PeerType, _PidList=[] ) ->
	text_utils:format( "peer type '~s' not associated to any peer",
					   [ PeerType ] );

multiple_peer_type_to_string( PeerType, PidList ) ->
	text_utils:format( "peer type '~s' associated to peers ~w",
					   [ PeerType, PidList ] ).
...
%
-spec peer_to_name_table_to_string( peer_to_name_table() ) -> string().
peer_to_name_table_to_string( PeerToNameTable ) ->
	...
	case table:enumerate( PeerToNameTable ) of
		...
		Entries ->
			...
			text_utils:format( "~B (unique) peers defined an attribute: ~s",
							   [ length( Entries ),
								 text_utils:strings_to_string( Strings ) ] )
...
peer_attribute_name_to_string( PeerType, AttributeName ) ->
	text_utils:format( "peer type '~s' associated to dataflow attribute name "
					   "'~s'", [ PeerType, AttributeName ] ).
...
%
-spec attribute_to_string( input_port_name(), input_port()  ) -> string().
attribute_to_string( InputPortName, #input_port{ value_status={ set, V } } ) ->
	text_utils:format( "attribute '~s' is set to value '~p'",
					   [ InputPortName, V ] );

attribute_to_string( InputPortName, #input_port{ value_status=unset } ) ->
	text_utils:format( "attribute '~s' is unset", [ InputPortName ] ).
...
attribute_update_to_string( AttributeName, AttributeValue ) ->
 text_utils:format( "attribute '~s' being set to value '~p'",
...
% object.
%
-spec attributes_to_string( wooper:state() ) -> string().
attributes_to_string( State ) ->
	...
	text_utils:format( "~B attributes:~s", [ length( AttrStrings ),
				text_utils:strings_to_string( AttrStrings ) ] ).
...
trace_initial_state( DataflowAttributeSpecs, InitialAttributeValues, State ) ->
	...
	?info_fmt( "Registering following dataflow attributes:~s~n"
			   "with following respective initial values:~s",
			   [ text_utils:strings_to_enumerated_string( AttrStrings ),
				 text_utils:strings_to_enumerated_string( ValueStrings ) ] ),

	?debug_fmt( "Full details at the creation of this dataflow object: ~s",
				[ class_DataflowBlock:to_string( State ) ] ).



% Helps to create attributes with different modalities / dimensions.
% Based on the required properties to build an attribute,
% creates as many attributes [foo_A, foo_B, foo_C] for a dimension [A, B, C]
%
...
create_attribute_specs_with_dimension( AttributeName, Semantics, Unit,
	...
	[ #dataflow_attribute_spec{
			attribute_name=text_utils:format( "~s[~s]", [ AttributeName, D ] ),
			semantics=Semantics,
			unit=Unit,
			type_description=TypeDescription,
...
decode_initial_attribute_values( AttributeSpecs, NameValueAttrPairs, State ) ->
	...
	?debug_fmt( "Expected ~B attribute names:~s~nand received ~B values, "
				"for attribute names:~s.",
				[ ExpectedAttrCount,
				  text_utils:strings_to_string( ExpectedAttrNames ),
				  ProvidedAttrCount,
	...
	case set_utils:is_empty( MissingElements ) of
		...
		false ->
			...
			MissingStrings = [ text_utils:format( "~s", [ S ] )
			...
			case ?getAttr(accept_non_valued_attrs) of
				...
				true ->
					?info_fmt( "Following ~B attributes have no initial value, "
							   "hence will start as unset:~s",
							   [ MissingCount, MissingMessage ] );

				false ->
					?error_fmt( "Following ~B attributes have no initial value "
								"(which is not accepted here):~s",
								[ MissingCount, MissingMessage ] ),
	...
	InitialValues = [ list_hashtable:getValueWithDefaults( _K=AttrName,
						_DefaultValue=undefined, _Table=NameValueAttrPairs )
					  || AttrName <- ExpectedAttrNames ],

	?debug_fmt( "Initial value setting:~s", [ text_utils:strings_to_string(
		 [ text_utils:format( "attribute '~s' set to ~p", [ N, V ] )
...
check_expected_attributes( ExpectedAttrNamesSet, ExpectedAttrNames,
	...
	case set_utils:size( ExpectedAttrNamesSet ) < ExpectedAttrCount of
		...
		true ->
			...
			DuplicateStrings = [ text_utils:format( "attribute '~s' listed ~B "
									"times", [ AttrName, Count ] )
	...
	case set_utils:size( ProvidedAttrNamesSet ) < ProvidedAttrCount of
		...
		true ->
			...
			DuplicateStrings = [ text_utils:format(
						   "attribute '~s' set ~B times", [ AttrName, Count ] )
								 || { AttrName, Count } <-  DuplicatePairs ],

			Message = text_utils:format( "At least one attribute received "
						"more than one initial value:~s.",
						[ text_utils:strings_to_string( DuplicateStrings ) ] ),
...
-spec check_no_extra_initial_values( set_utils:set( dataflow_attribute_name() ),
									 set_utils:set( dataflow_attribute_name() ),
									 wooper:state() ) -> basic_utils:void().
check_no_extra_initial_values( ProvidedAttrNamesSet, ExpectedAttrNamesSet,
							   State ) ->
	...
	case set_utils:is_empty( ExtraAttrNames ) of
		...
		false ->
			...
			AttrNames = [ text_utils:format( "~s", [ N ] ) || N <- AttrList ],
