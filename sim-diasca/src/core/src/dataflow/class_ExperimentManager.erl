...
%
-define( wooper_method_export, registerDataflow/1,
		 registerExperimentEntryPoint/1, registerExperimentExitPoint/1,
		 registerUnitManager/3, onFirstDiasca/2,
		 notifyFullyCompletedChangeset/3, onEventProcessed/3 ).
...
% to happen, i.e. in practice of all the instances of dataflows and all dataflow
...
% triggered:
%
-type creation_match_table() :: table:table( creation_event_match(),
											 [ unit_manager_pid() ] ).
...
% be triggered:
%
-type destruction_match_table() :: table:table( destruction_event_match(),
												[ unit_manager_pid() ] ).
...
%
-type association_match_table() :: table:table( association_event_match(),
												[ unit_manager_pid() ] ).
...
% manager(s) shall be triggered:
%
-type binary_association_match_table() :: table:table(
...
% shall be triggered:
%
-type disassociation_match_table() :: table:table( disassociation_event_match(),
												   [ unit_manager_pid() ] ).
...
% be triggered:
%
-type connection_match_table() :: table:table( connection_event_match(),
											   [ unit_manager_pid() ] ).
...
% shall be triggered:
%
-type disconnection_match_table() :: table:table( disconnection_event_match(),
												  [ unit_manager_pid() ] ).
...
% triggered:
%
-type update_match_table() :: table:table( update_event_match(),
										   [ unit_manager_pid() ] ).
...
%
-type ack_table() :: table:table( unit_manager_pid(), [ event_id() ] ).
...
% the connection if a unit created because of the previous event. Even in the
...
%
% - unit_managers :: table:table( unit_manager_pid(), [ dataflow_unit_type() ] )
% is a table whose keys are the PID of the known unit managers, and whose
% associated values are the list of types of dataflow units that each manages
%
...
% of the entry point (if any) for all dataflows managed by this experiment
% manager
%
% - exit_point_pid :: basic_utils:maybe( dataflow_exit_point_pid() ) is the PID
% of the exit point (if any) for all dataflows managed by this experiment
% manager
%
% - match_tables :: match_tables() is a record holding the various per-type of
% events match tables in use, namely with the following fields:
%
%   * any_match_table :: any_match_table() is a list of the unit managers that
%   subscribed to any type of dataflow synchronization event
%
%   * creation_match_table :: creation_match_table() allows to determine, for an
%   event clause regarding the creation of a dataflow object that matches, the
%   corresponding list of unit managers that shall be triggered
%
%   * destruction_match_table :: destruction_match_table() allows to determine,
%   for an event clause regarding the destruction of a dataflow object that
%   matches, the corresponding list of unit managers that shall be triggered
%
%   * association_match_table :: association_match_table() allows to determine,
%   for an event clause regarding the (non-binary) association of a dataflow
%   object that matches, the corresponding list of unit managers that shall be
%   triggered
%
%   * binary_association_match_table :: binary_association_match_table() allows
%   to determine, for an event clause regarding the binary association of a
%   dataflow object that matches, the corresponding list of unit managers that
%   shall be triggered
%
%   * disassociation_match_table :: disassociation_match_table() allows to
%   determine, for an event clause regarding the disassociation of a dataflow
%   object that matches, the corresponding list of unit managers that shall be
%   triggered
%
%   * connection_match_table :: connection_match_table() allows to determine,
%   for an event clause regarding the connection of a dataflow object that
%   matches, the corresponding list of unit managers that shall be triggered
%
%   * disconnection_match_table :: disconnection_match_table() allows to
%   determine, for an event clause regarding the disconnection of a dataflow
%   object that matches, the corresponding list of unit managers that shall be
%   triggered
%
%   * update_match_table :: update_match_table() allows to determine, for an
%   event clause regarding the update of a dataflow object that matches, the
%   corresponding list of unit managers that shall be triggered
%
% - pending_events :: [ world_event() ] is the (ordered) list of the next events
% that will be dispatched once the current one will be fully processed
%
% - waited_event_acks :: ack_table() is a table associating to each unit manager
% a list of the identifiers of the events that have been notified to this
% manager and that have not been acknowledged yet
...
destruct( State ) ->
	...
	?trace_fmt( "Being deleted (the ~B registered unit managers will be "
	...
	%
	%[ UnitManagerPid ! delete || UnitManagerPid <-
	%     table:size( ?getAttr(unit_managers) ) ],
	%
...
registerDataflow( State ) ->
	...
	case lists:member( DataflowPid, KnownDataflows ) of
		...
		false ->
			?trace_fmt( "Registered dataflow ~p.", [ DataflowPid ] ),

			NewState = setAttribute( State, dataflows,
									 [ DataflowPid | KnownDataflows ] ),

			?wooper_return_state_result( NewState, dataflow_registered )
...
registerUnitManager( State, ManagedUnitTypes, EventMatches ) ->
	...
	?debug_fmt( "Registering unit manager ~w, in charge of following ~B "
				"types of units: ~w, and having declared ~s.", [ UnitManagerPid,
					length( ManagedUnitTypes ),
					ManagedUnitTypes,
					class_DataflowUnitManager:event_clauses_to_string(
					  EventMatches ) ] ),
	...
	NewUnitManagers = case lists:member( UnitManagerPid,
		...
		false ->
			table:addEntry( UnitManagerPid, ManagedUnitTypes, UnitManagers )

	end,

	% Starts with no event waited:
	NewAckTable = table:addNewEntry( UnitManagerPid, [],
	...
	?wooper_return_state_result( NewState, unit_manager_registered ).
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, creation_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewCreationTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, destruction_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewDestructionTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, association_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewAssocTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid, State )
  when is_record( Clause, binary_association_event_match ) ->
	...
	NewBinAssocTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, disassociation_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewDisassocTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, connection_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewConnTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
				State ) when is_record( Clause, disconnection_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewDisconnTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
register_event_matches( _EventMatches=[ Clause | T ], UnitManagerPid,
						State ) when is_record( Clause, update_event_match ) ->

	MatchTables = ?getAttr(match_tables),

	NewUpdateTable = table:appendToEntry( _K=Clause, UnitManagerPid,
...
% Notifies (most probably from the world manager) this experiment manager that a
% fully completed changeset has been applied on the state-of-the-world side, and
% thus is ready to be taken into account by unit managers, on the computation
% side - once the corresponding events will be dispatched adequately, by this
% oneway.
%
% (actor oneway)
%
...
notifyFullyCompletedChangeset( State, Changeset, _SenderPid ) ->
	...
	?info_fmt( "Dispatching following changeset to the known "
			   "unit managers: ~s",
	...
	case dispatch_next_event( Changeset, State ) of
		...
		{ DispatchedState, _IsWaiting=true } ->
			...
			%
			?debug_fmt( "Processing of this changeset initiated; new state: ~s",
						[ to_string( DispatchedState ) ] ),

			?wooper_return_state_only( DispatchedState );
		...
		{ DispatchedState, _IsWaiting=false } ->
			...
			?info( "Registered unit managers did not subscribe to any world "
				   "event in this changeset, so resuming dataflows directly." ),

			?wooper_return_state_only( resume_dataflows( DispatchedState ) )
...
% (helper)
%
-spec check_no_past_event_waited( wooper:state()  ) -> basic_utils:void().
check_no_past_event_waited( State ) ->
...
get_waited_events( _AckPairs=[ { UnitManagerPid, WaitedEventIds } | T ],
	...
	Str = text_utils:format( "unit manager ~w still waiting for ~B events, "
							 ...
							 [ UnitManagerPid, length( WaitedEventIds ),
							   WaitedEventIds ] ),
...
onEventProcessed( State, EventId, UnitManagerPid ) ->
	...
	WaitedEvents = table:getEntry( _K=UnitManagerPid, AckTable ),
	...
	NewAckTable = table:addEntry( UnitManagerPid, RemainingEvents, AckTable ),
		...
		true ->
			...
			case dispatch_next_event( ?getAttr(pending_events), AckState ) of
				...
				{ DispatchedState, _IsWaiting=false } ->
					...
					?info( "All current events have been processed by their "
		...
		true ->
			?void_fmt( "Dispatched ~s.",
				[ dataflow_support:world_event_to_string(
					DestructionEvent ) ] ),
			DispatchedState = setAttribute( DestructionState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
	...
	case AnyWaiting or AssocWaiting of
		...
		true ->
			?void_fmt( "Dispatched ~s.",
				[ dataflow_support:world_event_to_string(
					AssociationEvent ) ] ),
			DispatchedState = setAttribute( AssociationState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
	...
	case AnyWaiting or AssocWaiting of
		...
		true ->
			?void_fmt( "Dispatched ~s.",
				[ dataflow_support:world_event_to_string(
					BinAssociationEvent ) ] ),
			DispatchedState = setAttribute( BinAssocState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
...
dispatch_next_event( _Changeset=[ DisassociationEvent=#disassociation_event{}
								  | T ], MatchTables, State ) ->
	...
	case AnyWaiting or DisassocWaiting of
		...
		true ->
			?void_fmt( "Dispatched ~s.",
				[ dataflow_support:world_event_to_string(
					DisassociationEvent ) ] ),
			DispatchedState = setAttribute( DisassociationState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
...
dispatch_next_event( _Changeset=[ ConnectionEvent=#connection_event{} | T ],
	...
	case AnyWaiting or ConnWaiting of
		...
		true ->
			...
			DispatchedState = setAttribute( ConnectionState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
...
dispatch_next_event( _Changeset=[ DisconnectionEvent=#disconnection_event{}
								  | T ], MatchTables, State ) ->
	...
	case AnyWaiting or DisconnWaiting of
		...
		true ->
			?void_fmt( "Dispatched ~s.",
				[ dataflow_support:world_event_to_string(
					DisconnectionEvent ) ] ),
			DispatchedState = setAttribute( DisconnectionState, pending_events,
											T ),
			{ DispatchedState, _IsWaiting=true };
...
dispatch_creation_event( CreationEvent=#creation_event{
	...
	case object_type_match( ObjectType, ObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Creation event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				CreationEvent ) ] ),
			Param
...
dispatch_destruction_event( DestructionEvent=#destruction_event{
	...
	case object_type_match( ObjectType, ObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Destruction event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				DestructionEvent ) ] ),
			Param
...
dispatch_association_event( AssociationEvent=#association_event{
	...
	case object_type_match( ObjectType, ObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Association event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				AssociationEvent ) ] ),
			Param
...
dispatch_disassociation_event( DisassociationEvent=#disassociation_event{
	...
	case object_type_match( ObjectType, ObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Disassociation event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				DisassociationEvent ) ] ),
			Param
...
dispatch_connection_event( ConnectionEvent=#connection_event{
							 ...
							 source_external_id=SourceExternalId,
							 target_external_id=TargetExternalId,
							 % Non-matched: source_object_pid=ObjectPid,
							 % Non-matched: target_object_pid=ObjectPid,
							 source_attribute_name=SourceAttrName,
							 target_attribute_name=TargetAttrName,
							 dataflow_pid=DataflowPid },
						   #connection_event_match{
							 ...
							 source_external_id_match=SourceExternalIdMatch,
							 target_external_id_match=TargetExternalIdMatch,
							 source_attribute_name_match=SourceAttrNameMatch,
							 target_attribute_name_match=TargetAttrNameMatch,
							 dataflow_pid_match=DataflowPidMatch },
						   UnitManagers, Param={ State, IsWaiting } ) ->
	case        object_type_match( SourceObjectType, SourceObjectTypeMatch )
		andalso object_type_match( TargetObjectType, TargetObjectTypeMatch )
		andalso external_id_match( SourceExternalId, SourceExternalIdMatch )
		andalso external_id_match( TargetExternalId, TargetExternalIdMatch )
		andalso attribute_name_match( SourceAttrName, SourceAttrNameMatch )
		andalso attribute_name_match( TargetAttrName, TargetAttrNameMatch )
		andalso dataflow_pid_match( DataflowPid, DataflowPidMatch ) of
		...
		false ->
			%?debug_fmt( "Connection event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				ConnectionEvent ) ] ),
			Param
...
dispatch_disconnection_event( DisconnectionEvent=#disconnection_event{
	...
	case        object_type_match( SourceObjectType, SourceObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Disconnection event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				DisconnectionEvent ) ] ),
			Param
...
dispatch_update_event( UpdateEvent=#update_event{
	...
	case object_type_match( ObjectType, ObjectTypeMatch )
		...
		false ->
			%?debug_fmt( "Update event not matched: ~s.",
			%			[ dataflow_support:world_event_to_string(
			%				UpdateEvent ) ] ),
			Param
...
register_waited_ack( EventId, UnitManagerPid, State ) ->
	...
	NewAcks = table:appendToExistingEntry( _K=UnitManagerPid, _V=EventId,
										   ?getAttr(waited_event_acks) ),
...
% (match helper)
%
-spec attribute_name_match( dataflow_attribute_name(),
							attribute_name_match() ) -> boolean().
attribute_name_match( _AttributeName,
					  _AttributeNameMatch=any_attribute_name ) ->
	true;

attribute_name_match( AttributeName, _AttributeNameMatch=AttributeName ) ->
	true;

attribute_name_match( _AttributeName, _AttributeNameMatch ) ->
	false.
...
to_string( State ) ->
	...
	text_utils:format( "experiment manager ~s, ~s, ~s, ~s, ~s, ~s, ~s and ~s",
					   [ WorldString, DataflowString, EntryPointString,
...
match_tables_to_string( State ) ->
	...
	CreationString = case length( CreationList ) of
		...
		CreationLen ->
			...
			text_utils:format( "following ~B creation clauses registered:~s",
	...
	DestructionString = case length( DestructionList ) of
		...
		DestructionLen ->
			...
			text_utils:format( "following ~B destruction clauses registered:~s",
	...
	AssociationString = case length( AssociationList ) of
		...
		AssociationLen ->
			...
			text_utils:format( "following ~B association clauses registered:~s",
							   [ AssociationLen, AssociationBulletString ] )
	...
	BinAssocString = case length( BinAssocList ) of
		...
		BinAssocLen ->
			...
			text_utils:format(
			  "following ~B binary association clauses registered:~s",
	...
	DisassociationString = case length( DisassociationList ) of
		...
		DisassociationLen ->
			...
			text_utils:format( "following ~B disassociation clauses "
							   "registered:~s", [ DisassociationLen,
												  DisassociationBulletString ] )
	...
	ConnectionString = case length( ConnectionList ) of
		...
		ConnectionLen ->
			...
			text_utils:format( "following ~B connection clauses registered:~s",
							   [ ConnectionLen, ConnectionBulletString ] )
	...
	DisconnectionString = case length( DisconnectionList ) of
		...
		DisconnectionLen ->
			...
			text_utils:format( "following ~B disconnection clauses "
							   "registered:~s", [ DisconnectionLen,
												  DisconnectionBulletString ] )
	...
	UpdateString = case length( UpdateList ) of
		...
		UpdateLen ->
			...
			text_utils:format( "following ~B update clauses registered:~s",
	...
	text_utils:format( "regarding synchronization events:~s",
...
event_clause_to_string( EventMatch, UnitManagers ) ->
	text_utils:format( "synchronization events matching ~s will be routed "
					   "to following ~B unit manager(s): ~w",
					   [ class_DataflowUnitManager:event_clause_to_string(
						   EventMatch ),
