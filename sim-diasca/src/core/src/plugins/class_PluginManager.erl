...
-define( wooper_static_method_export, notify/1, notify_if_registered/1,
		 notify_simulator_start/0, notify/2, notify_case_specific/2 ).
...
construct( State, ?wooper_construct_parameters ) ->
	...
	naming_utils:register_as( ?plugin_manager_name, ?registration_type ),

	?send_info_fmt( TraceState, "Initialising plugin manager, "
...
destruct( State ) ->
	...
	naming_utils:unregister( ?plugin_manager_name, ?registration_type ),
...
notify( Event ) ->
	...
	PluginManagerPid = naming_utils:get_registered_pid_for(
						 ?plugin_manager_name, _Scope=?look_up_type ),
...
% (static)
%
-spec notify_if_registered( sim_diasca_plugin:plugin_event() ) ->
								  basic_utils:void().
notify_if_registered( Event ) ->

	case naming_utils:is_registered( ?plugin_manager_name,
									 _Scope=?look_up_type ) of
