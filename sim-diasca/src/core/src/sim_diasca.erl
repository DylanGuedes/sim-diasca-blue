...
-export([ init/1, init/2, init/3, is_running/0,
...
get_simulation_identifiers() ->
	...
	SII = case init:get_argument( '-simulation-instance-id' ) of
		...
		{ ok, [ UserSpecifiedSII ] } when is_list( UserSpecifiedSII ) andalso
				  ...
				  ?notify_info_fmt_cat( "Using simulation UUID '~s', and "
										"user-specified SII '~s'.",
		...
		error ->
				  ...
				  % numerical characters, instead of 36 alphanumerical ones for
				  % the UUID):
				  %
				  DeducedSII = text_utils:integer_to_string(
								 erlang:phash2( UUID ) ),

				  ?notify_info_fmt_cat( "Using simulation UUID '~s', and, "
										"no SII having been specified by the "
										"user, a UUID-derived one, SII '~s'.",
										[ UUID, DeducedSII ],
										EmitterCategorization ),
