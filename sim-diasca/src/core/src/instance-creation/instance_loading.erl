		...
		fun( Filename, AccFilenames ) ->
			...
			case filename:extension( Filename ) of
				...
				CompressExt ->
					...
					io:format( "Decompressing '~s'.~n", [ Filename ] ),
...
initialisation_waiting_loop( UserIdResolverPid, CreationCount, Creators,
	...
	Message = io_lib:format( "~nAll ~B initial instances have been "
							 "successfully loaded from the initialisation "
							 "sources, in ~s.~n~n",
...
initialisation_waiting_loop( UserIdResolverPid, CreationCount, Creators,
	...
	receive
		...
		{ initialisation_file_read, InstanceCreationCount, ReaderPid } ->
			...
			?notify_debug_fmt_cat( "Instance reader ~p terminated.",
...
read_all_lines( File, ChunkCount, Creators, Filename ) ->
	...
	?notify_debug_fmt_cat( "Reading all lines from '~s'.", [ Filename ],
						   ?loader_cat ),
	...
	case read_chunk( File, ChunkCount, LineCount ) of
		...
		{ _Chunk=[], LineCount } ->
			...
			?notify_debug_fmt_cat(
			   "Read all ~B lines from '~s', resulting in ~B "
			   "instance creations.",
		...
		{ Chunk, NewLineCount } ->
			...
			?notify_debug_fmt_cat( "Instance reader has a new chunk of "
								   "~B lines to process.", [ length( Chunk ) ],
								   ?loader_cat ),
	...
	case table:foldOnEntries( CheckFun, _InitialIdAcc=[], IdTable ) of
		...
		UndefinedIds ->
			...
			?notify_error_fmt_cat(
			   ...
			   [ length( UndefinedIds ),
				 text_utils:binaries_to_string( UndefinedIds ) ], ?loader_cat ),
