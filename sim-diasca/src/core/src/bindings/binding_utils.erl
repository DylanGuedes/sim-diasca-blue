...
get_binding_manager( Language ) ->
	...
	apply( BindingManagerClass, get_registered_manager, [] ).
...
% (helper)
%
-spec set_binding_managers_record( [ { language(),
									   binding_manager_pid() } ] ) ->
										 binding_managers().
...
set_binding_managers_record( BindingManagerPairs )
  when is_list( BindingManagerPairs ) ->
	...
	PythonBindingManagerPid = case table:lookupEntry( python,
													  BindingsTable ) of
	...
	JavaBindingManagerPid = case table:lookupEntry( java, BindingsTable ) of
...
% take the form of a 2-tuple { ClassName, Language }). This availability is
% tested through the presence of a valid PID in a binding_managers record.
%
% (helper)
%
-spec check_implementation_language( [ wooper:classname() |
									   { wooper:classname(), language() } ],
									 binding_managers() ) -> ok.
