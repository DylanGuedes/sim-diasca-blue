...
% Test of Actor class, regarding time management, communication, actor creation,
% etc.
%
...
-define( wooper_method_export, onFirstDiasca/2, actSpontaneous/1,
		 lowerTraceIntensity/1, onActorCreated/4,
...
% OnFirstDiasca/2).
%
-type creation_policy() :: 'no_creation' | 'initial_creation'
...
construct( State, ?wooper_construct_parameters ) ->
	...
	%io:format( "Creating a class_TestActor ~p with parameters:~n~p.~n",
	...
	CreatedActors = case CreationPolicy of
		...
		initial_creation ->
			...
			%io:format( "Creating circular '~s'.~n", [ InitialName ] ),

			CircularPid = class_Actor:create_initial_actor(
							class_TestCircularActor,
							[ InitialName, "My Message" ] ),

			%io:format( "Created circular ~p.~n", [ CircularPid ] ),
	...
	CreatedState = case CreationPolicy of
		...
		creation_from_constructor ->
			...
			%io:format( "Creating circular '~s'.~n", [ RuntimeName ] ),
	...
	?send_info_fmt( TraceState, "Creating a new test actor (PID: ~w, AAI: ~B, "
					"seed: ~w), "
...
actSpontaneous( State ) ->
	...
	%?info_fmt( "actSpontaneous: local agenda is ~w.",
...
manage_actor_creation( CurrentOffset, State ) ->
	...
	case ?getAttr(creation_countdown) of
		...
		NonNullCount ->
			...
			%io:format( "(this actor will create an actor in ~B activations)~n",
			%		   [ NextCount ] ),
...
% (actor oneway)
%
-spec onActorCreated( wooper:state(), pid(), class_Actor:tag(), pid() ) ->
							oneway_return().
...
hello( State, SenderName, SenderPid ) ->
	...
	%	[ SenderName, SenderPid, CurrentOffset ], State ),
