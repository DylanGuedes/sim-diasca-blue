...
-define( wooper_method_export,
		 ...
		 setInitialSimulationTimestamp/2, setInitialSimulationTimestamp/3,

		 getFinalTick/1, setFinalTick/2,
		 setFinalSimulationTimestamp/2, setFinalSimulationTimestamp/3,

		 setSimulationTimeframe/3,
...
-export_type([ tick/0, tick_offset/0, diasca/0, logical_timestamp/0,
			   simulation_events/0, interactivity_mode/0, time_manager_pid/0 ]).
...
% To additionally discriminate between PID having the same middle number, based
% on node:
%
%-define( display_console( FormatString, Values ),
%		 trace_utils:debug_fmt( "[~w on ~s] " ++ FormatString,
%								[ self(), node() | Values ] ) ).
...
% that are to keep track of the logical scheduling (i.e. ticks and diascas),
...
construct( State, SimulationTickDuration, InteractivityMode,
	...
	TickWaitingDuration = case InteractivityMode of
			...
			case round( ?scale_factor_for_interactive_time * 1000
				...
				0 ->
					?send_warning_fmt( CategorizedState,
						"Running in interactive mode, with a inter-tick "
						"duration that has been forced to exactly 1 ms, "
						"knowing that applying the interactive scale factor "
						"of x~f to the simulation tick duration "
						"(of ~fs, i.e. ~s) would have lead to a null "
						"inter-tick duration.",
				...
				RoundDuration ->
					?send_trace_fmt( CategorizedState, "Running in interactive "
						"mode, with an actual inter-tick duration of ~B ms "
						"(i.e. ~s), resulting from a simulation tick of ~f ms "
						"(i.e. ~s) to which the interactive scale factor of "
						"x~p applies.",
						[ RoundDuration, text_utils:duration_to_string(
							 round( RoundDuration ) ),
						  ActualSimulationTickDuration, TickDurationString,
						  ?scale_factor_for_interactive_time ] ),
					RoundDuration
...
% (oneway)
%
-spec setInitialSimulationTimestamp( wooper:state(), time_utils:timestamp() ) ->
										   oneway_return().
...
setInitialSimulationTimestamp( State, Date, Time ) ->
	...
	SetState = set_initial_simulation_timestamp( _Timestamp={ Date, Time },
												 State ),
...
% (helper)
%
-spec set_initial_simulation_timestamp( time_utils:timestamp(),
...
set_initial_simulation_timestamp( InitialTimestamp, State ) ->
	...
	?info_fmt( "New initial simulation tick is #~B, "
			   "deriving from specified initial timestamp at ~s.",
			   [ NewInitialTick,
...
% (oneway)
%
-spec setFinalSimulationTimestamp( wooper:state(), time_utils:timetamp() ) ->
										 oneway_return().
...
% (oneway)
%
-spec setSimulationTimeframe( wooper:state(), time_utils:timestamp(),
							  time_utils:timestamp() ) -> oneway_return().
...
beginTimeManagerTick( State, NewTickOffset ) ->
	...
	?display_console( "beginTimeManagerTick: new tick offset is #~B, "
					  "next action ~p.",
					  [ NewTickOffset, ?getAttr(next_action) ] ),
...
notifySpontaneousSubtreeCompletion( State, SubtreeTickOffset, ChildManagerPid,
	...
	?wooper_return_state_only( manage_possible_end_of_diasca( WaitedState ) ).
...
timerTickFinished( State ) ->
	...
	case ?getAttr(waited_count) of
		...
		0 ->
			...
			?wooper_return_state_only( TimerState );
	...
	case is_root_manager( State ) of
		...
		true ->
			?info_fmt( "Simulation-time milestone triggered at tick offset #~B,"
					   " while current wall-clock time is ~s.",
...
settings_to_string( #simulation_settings{
	...
	text_utils:strings_to_string( [ NameString, DurationString,
...
watchdog_main_loop( TimeManagerPid, DelayBeforeFirstStall,
	...
	receive
		...
		suspendWatchdog ->
			...
			trace_utils:info_fmt( "Watchdog suspended." ),
			...
			receive
				...
				resumeWatchdog ->
					...
					trace_utils:info_fmt( "Watchdog resumed." ),
		...
		case AccumulatedDuration of
			...
			TooLong when TooLong > DelayBeforeFailed ->
				...
				?notify_fatal_fmt_cat(
				   ...
				   "deemed orphaned, shutting it down now at ~s.",
			...
			_OtherDuration ->
				...
				?display_console( " ### Watchdog detected a stalled simulation "
						   "at ~s, see simulation traces for diagnosis.",
						   [ time_utils:get_textual_timestamp() ] ),
...
time_tracker_main_loop( PreviousDisplayTime, PreviousSimTimestamp,
	...
	after WaitTime ->
			...
			%?display_console( "after triggered, at ~s from ~B~n",
...
display_diasca_in_progress( SimTimestamp ) ->
	...
	BaseText = text_utils:format( "diasca ~p still in progress at ~s",
...
-spec get_full_textual_timings( tick_offset(), diasca(), wooper:state() ) ->
				{ string(), timing_info(), time_utils:time() }.
...
% (helper)
%
-spec format_real_time_date() -> { string(), string(), time_utils:time() }.
...
init( State ) ->
	...
	PostStartedState = setAttributes( StartedState, [
		...
		% batch mode, and since, in interactive mode, the timer main loop may
		% send a timerTickFinished message even before the first
...
schedule_new_tick( NextTickOffset, State ) ->
	...
	case ?getAttr(interactivity_mode) of
		...
		interactive ->
			...
			% with a timerTickFinished call), except resetting the safeguard
			% against multi-tick scheduling:
			%
...
display_waiting_reason( State ) ->
	...
	io:format( "At tick offset #~B, time manager ~w waiting for "
			   ...
			   "~B child manager(s) and for ~s watchdog.",
...
manage_new_tick( NewTickOffset, State ) ->
	...
	?display_console( "Total initial wait summary: ~B, i.e. S=~B, C=~B, W=~B.",
					  [ WaitedCount, SpontaneousCount, ChildManagerCount,
						WaitedWatchdog ] ),

	?display_console( "~s: ~s is sent to ~B child time manager(s) "
					  "and to ~B local actor(s) that are scheduled for their "
					  "spontaneous behaviour (waited count, "
					  "including any watchdog: ~B).",
					  [ TickHeader, get_textual_timings( State ),
...
launch_timer( State ) ->
	...
	?notify_mute_fmt( "Starting global simulation clock in interactive mode "
					  ...
					  "of approximately ~fHz (period of exactly ~s), with "
					  "~B ms expected between simulation ticks.",
					  [ get_textual_timings( State ), Frequency,
						TickDurationString, TimerTimeOut ] ),
