...
-spec registerActor( wooper:state(), class_Actor:aai(),
		class_Actor:internal_name(), actor_pid(), class_name() ) ->
						   oneway_return().
...
% (oneway)
%
-spec unregisterActor( wooper:state(), actor_pid(), class_name() ) ->
							 oneway_return().
...
% (const request)
%
-spec getActorInformationLocal( wooper:state(), actor_pid() ) -> request_return(
...
%
-spec get_root_tracker() -> instance_tracker_pid().
get_root_tracker() ->
	naming_utils:get_registered_pid_for( get_registration_name(), global ).
...
% (static method, to be used by clients of the instance tracker)
%
-spec get_local_tracker() -> instance_tracker_pid().
get_local_tracker() ->
	naming_utils:get_registered_pid_for( get_registration_name(), local ).
...
-spec is_local_tracker_registered() ->
								   instance_tracker_pid() | 'not_registered'.
is_local_tracker_registered() ->
	naming_utils:is_registered( get_registration_name(), local ).
...
%
-spec actor_info_to_string( actor_info(), actor_pid() ) -> string().
...
agent_table_to_string( Table ) ->
	...
	AgentStrings = lists:foldl(
				fun( { _K=Pid, _V=AgentRef }, Acc ) ->
					[ io_lib:format( "agent reference ~p associated to PID ~p",
									 [ AgentRef, Pid ] ) | Acc ]
				end,
				_InitialAcc=[],
				InstancePairs ),
