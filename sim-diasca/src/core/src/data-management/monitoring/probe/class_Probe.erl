...
-define( trace_emitter_categorization, "Core.Probe.Basic" ).
...
%  should have the same name, otherwise they would end up writing in the same
%  files and corrupting them (so the result manager will preventively reject
%  such attempts)
%
%  - or { Name, ProbeOptions } where ProbeOptions is a probe_options() record
%
...
construct( State, { NameInit, ProbeOptions }, CurveNames, Zones, Title,
  ...
  when is_record( ProbeOptions, probe_options ) ->
	...
	%basic_utils:display( "Creating probe ~s", [ ProbeName ] ),

	% First the direct mother classes:

	ProducerState = class_ResultProducer:construct( State,
											?trace_categorize( ProbeName ) ),
	...
	getAttribute( ProducerState, result_manager_pid ) !
				{ declareProbe, [ text_utils:string_to_binary( ProbeName ),
	...
	receive
		...
		{ wooper_result, output_requested } ->
			...
			?send_trace_fmt( DeferredState, "The probe '~s' will produce an "
							 "expected result.~n", [ ProbeName ] ),
...
construct( State, Name, CurveNames, Zones, Title, XLabel, YLabel, MetaData ) ->
	...
	construct( State, { Name, _DefaultOptions=#probe_options{} }, CurveNames,
			   Zones, Title, XLabel, YLabel, MetaData ).
...
sendResults( State, [ data_only ] ) ->
	...
	case ?getAttr(sample_count) of
		...
		0 ->
			?info_fmt( "The probe '~s' did not receive any data sample.",
					   [ Name ] );
...
sendResults( State, [ plot_only ] ) ->
	...
	case ?getAttr(sample_count) of
		...
		0 ->
			?info_fmt( "The probe '~s' did not receive any data sample, hence "
...
create_facility_probe( { Name, Options }, CurveEntries, ZoneEntries, Title,
	...
	%
	NewOptions = Options#probe_options{
...
interpret_options( _ProbeOptions=#probe_options{
	...
	ProbeDir = case ProbeDirectory of
...
get_curve_command( { CurveIndex, BinCurveName }, _Settings ) ->
	...
	%
	% +1 to account for the abscissa column
	%
	io_lib:format( "~B title \"~s\" noenhanced", [ CurveIndex + 1, Title ] ).
...
generate_report( Name, State ) ->
	...
	case file_utils:is_existing_file( TargetFilename ) of
		...
		true ->
			?warning_fmt( "The file '~s' was already existing, "
	...
	OutputMessage = case system_utils:run_executable( Command,
		...
		{ _ReturnCode=0, CmdOutput } ->
			...
			?warning_fmt( "Report generation succeeded for '~s', but it output "
