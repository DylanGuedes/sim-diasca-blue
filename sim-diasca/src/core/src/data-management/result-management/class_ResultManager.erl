...
construct( State, ?wooper_construct_parameters ) ->
	...
	naming_utils:register_as( RegistrationName, ?registration_type ),
...
destruct( State ) ->
	...
	?debug_fmt( "Deleting ~B basic probes.", [ length( BasicProbePidList ) ] ),
...
isResultProducerWanted( State, ProducerName, Nature ) ->
	...
	%trace_utils:debug_fmt( "isResultProducerWanted for producer '~s' "
...
isResultProducerWantedWithOptions( State, ProducerName, Nature ) ->
	...
	%   "for producer '~s' of nature ~p: answer is '~p'.",
...
simulation_succeeded( State ) ->
	...
	ResultOneway= { results_collected,
					text_utils:string_to_binary( ResultDirName ) },
...
manage_all_producers( State ) ->
	...
	ManagedState = case ?getAttr(result_spec) of
		...
		_Other ->
			...
			%trace_utils:debug( "Managing result queues:~n" ),
...
trigger_producer( ProducerName, ProbeTable ) ->
	...
	%trace_utils:debug_fmt( "Triggering producer ~s (~w).",
...
update_queues_after_result( ProducerPid, PidToQueueTable, ProbeTable,
	...
	{ NewPending, NewWaited } = case Queue#result_queue.pending_results of
		...
		[ ProducerName | T ] ->
			...
			%trace_utils:debug_fmt( "Replenishing queue #~B with '~s'.",
	...
	case ?getAttr(result_found) of
		...
		true ->
			...
			case file_utils:filter_by_extensions( Files,
				...
				[] ->
					trace_utils:info( "(results matched the specification, "
									  "but none was graphical, thus nothing to "
									  "be browsed here)" );

				L ->
					trace_utils:info_fmt( "(displaying ~B graphical results)",
										  [ length( L ) ] ),
		...
		false ->
			trace_utils:info( "(no result matched the specification, "
...
browse_reports() ->
				...
				simulation_succeeded ->
					...
					receive
						...
						{ results_collected, ResultBinDir } ->
							trace_utils:info_fmt( "Results are available now, "
												  "in the '~s' directory.",
												  [ ResultBinDir ] )
		...
		false ->
			...
			receive
				...
				simulation_succeeded ->
					...
					trace_utils:info( "Simulation success, result reports to be"
								 " processed, collected then browsed now." ),
					...
					receive
						...
						{ wooper_result, results_browsed } ->
							?notify_info_cat( "Result reports have been "
			...
			after 5000 ->
					...
					trace_utils:info( "Simulation failed, no result gathered." )
	...
	case WaitForPerformanceTracker of
		...
		true ->
			...
			trace_utils:debug( "Waiting finally for the performance tracker" ),
			...
			receive
				...
				{ wooper_result, report_generated } ->
					...
					trace_utils:info( "Monitoring reports have been "
									  "successfully generated." )
...
compile( Pattern ) ->
	...
	%trace_utils:debug_fmt( "Compiling pattern '~s'.", [ Pattern ] ),
...
is_result_wanted( ProducerName, Nature, State ) ->
	...
	%trace_utils:debug_fmt( "Is result producer '~s' wanted? '~p'.",
...
check_targeted( ProducerName, [ { _BinPattern, MatchSpec, Opts } | T ] ) ->
	...
	%trace_utils:debug_fmt( "Checking '~s' against targeted pattern ~p.",
...
check_blacklisted( ProducerName, [ { _BinPattern, MatchSpec } | T ] ) ->
	...
	%trace_utils:debug_fmt( "Checking '~s' against blacklisted pattern ~p.",
	%		  [ ProducerName, BinPattern ] ),
...
display_queues( Queues ) ->
	...
	trace_utils:debug_fmt( "Result queues:~n ~s",
